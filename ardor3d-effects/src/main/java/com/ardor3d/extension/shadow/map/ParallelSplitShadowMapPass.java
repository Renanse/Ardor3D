...
public class ParallelSplitShadowMapPass extends Pass {
  ...
  /**
   * The blending to both discard the fragments that have been determined to be free of shadows and to
   * blend into the background scene.
   */
  ...
  /**
   * True if we want to factor in texturing to shadows; useful for casting shadows against
   * alpha-tested textures. Default is false.
   */
  private boolean _useSceneTexturing = false;

  /**
   * True if we want to use the culling set on the objects instead of always culling front face (which
   * is done for shadow precision). Default is false.
   */
  private boolean _useObjectCullFace = false;

  /**
   * When true (the default) the generated shadow map textures are drawn over the scene in a separate
   * blend pass. If false, the shadows are generated, but not applied.
   */
  ...
  public ParallelSplitShadowMapPass(final Light light, final int shadowMapSize, final int numOfSplits) {
    ...
    // When rendering and comparing the shadow map with the current depth, the result will be set to
    // alpha 1 if in
  ...
  public void init(final Renderer r) {
    ...
    // render states to use when rendering into the shadow map, no textures or colors are required since
    // we're only
    ...
    // try {
    // _pssmShader.setShader(ShaderType.Vertex, "pssm",
  ...
  public void updateShadowMaps(final Renderer r) {
    ...
    for (int iSplit = 0; iSplit < _numOfSplits; iSplit++) {
      ...
      if (_light instanceof DirectionalLight) {
        ...
        if (_drawDebug) {
          boundingSphere.setData(frustumBoundingSphere.getCenter(), 10, 10, frustumBoundingSphere.getRadius());
      ...
      if (_drawDebug) {
        drawFrustum(r, _shadowMapRenderer.getCamera(), new ColorRGBA(1, 1, (iSplit + 1) / (float) _numOfSplits, 1),
            (short) 0xFFFF, true);
      }
  ...
  private void updateShaderVariables() {
    ...
    // final float split2 = (float) (_pssmCam.getSplitDistances().length > 2 ?
    // _pssmCam.getSplitDistances()[2]
    // : 0f);
    // final float split3 = (float) (_pssmCam.getSplitDistances().length > 3 ?
    // _pssmCam.getSplitDistances()[3]
    // : 0f);
    // final float split4 = (float) (_pssmCam.getSplitDistances().length > 4 ?
    // _pssmCam.getSplitDistances()[4]
    // : 0f);
    //
  ...
  /**
   * Saving this around until we fully support a good solution for non-directional lights. Like dual
   ...
   */
  public OffsetState getShadowOffsetState() { return _shadowOffsetState; }

  /**
   *
   * @return the offset state used when drawing the shadow overlay pass.
   */
  public OffsetState getShadowPassOffsetState() { return _shadowPassOffsetState; }
   ...
   */
  public int getNumOfSplits() { return _numOfSplits; }

  /**
   * Sets the number of frustum splits and thus the number of shadow textures created by this pass.
   * More splits creates crisper shadows at the cost of increased texture memory.
   *
   * @param numOfSplits
   ...
   */
  public int getShadowMapSize() { return _shadowMapSize; }
   ...
   */
  public double getMaxShadowDistance() { return _pssmCam.getMaxFarPlaneDistance(); }
   ...
   */
  public double getMinimumLightDistance() { return _minimumLightDistance; }
   ...
   */
  public ReadOnlyColorRGBA getShadowColor() { return _shadowColor; }
   ...
   */
  public boolean isUpdateMainCamera() { return _updateMainCamera; }

  /**
   * @param updateMainCamera
   *          True (the default) if we want to copy the current rendering camera into this pass for
   *          use in shadow generation. False if we will manage our shadow camera elsewhere.
   * @see #getPssmCam()
   */
  public void setUpdateMainCamera(final boolean updateMainCamera) { _updateMainCamera = updateMainCamera; }

  /**
   * @return the drawDebug
   */
  public boolean isDrawDebug() { return _drawDebug; }

  /**
   * @param drawDebug
   *          True if we want to draw camera and light frustums for debugging purposes. Default is
   *          false.
   */
  public void setDrawDebug(final boolean drawDebug) { _drawDebug = drawDebug; }

  /**
   * @return the drawShaderDebug
   */
  public boolean isDrawShaderDebug() { return _drawShaderDebug; }

  /**
   * @param drawShaderDebug
   *          True if we want to draw debug colors over the shadows, showing which level they come
   *          from.
   */
  public void setDrawShaderDebug(final boolean drawShaderDebug) { _drawShaderDebug = drawShaderDebug; }

  /**
   * @return the useSceneTexturing
   */
  public boolean isUseSceneTexturing() { return _useSceneTexturing; }
  ...
  public boolean isUseObjectCullFace() { return _useObjectCullFace; }

  /**
   * @param useObjectCullFace
   *          True if we want to use the culling set on the objects instead of always culling front
   *          face (which is done for shadow precision). Default is false.
   */
  ...
  public boolean isRenderShadowedScene() { return _renderShadowedScene; }

  /**
   * @param renderShadowedScene
   *          When true (the default) the generated shadow map textures are drawn over the scene in a
   *          separate blend pass. If false, the shadows are generated, but not applied.
   */
   ...
   */
  public PSSMCamera getPssmCam() { return _pssmCam; }

  /**
   * @return the texture store format to use for the shadow textures on the next call to
   *         init/setNumOfSplits.
   */
  public TextureStoreFormat getShadowTextureStoreFormat() { return _shadowTextureStoreFormat; }

  /**
   * @param shadowTextureStoreFormat
   *          - the texture store format to use for the shadow textures. Only has an affect if called
   *          prior to calling init on this pass (or if called before calling
   *          {@link #setNumOfSplits(int)}).
   */
  ...
  public boolean isKeepMainShader() { return _keepMainShader; }

  public void setKeepMainShader(final boolean keepMainShader) { _keepMainShader = keepMainShader; }

  public void addBoundsReceiver(final Spatial spatial) {
    _boundsReceiver.add(spatial);
  }

  public void setFiltering(final Filter filter) { this.filter = filter; }

  public BlendState getDiscardShadowFragmentsBlendState() { return _discardShadowFragments; }
}
