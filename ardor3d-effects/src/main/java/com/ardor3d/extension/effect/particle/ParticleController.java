...
/**
 * <code>ParticleController</code> controls and maintains the parameters of a particle system over
 * time.
 */
public class ParticleController extends ComplexSpatialController<ParticleSystem> {
  ...
  /**
   * Update the particles managed by this manager. If any particles are "dead" recreate them at the
   * origin position (which may be a point, line or rectangle.)
   *
   * @param secondsPassed
  ...
  public void update(final double secondsPassed, final ParticleSystem particles) {
    ...
    if (_currentTime >= getMinTime() && _currentTime <= getMaxTime()) {
      ...
      if (_controlFlow) {
        ...
        _releaseParticles +=
            (particles.getReleaseRate() * _timePassed * (1.0 + _releaseVariance * (MathUtils.nextRandomFloat() - 0.5)));
  ...
  /**
   * Set how soon after the last update the manager will send updates to the particles. Defaults to
   * .01f (10ms)<br>
   * <br>
   * This means that if an update is called every 2ms (e.g. running at 500 FPS) the particles position
   * and stats will be updated every fifth frame with the elapsed time (in this case, 10ms) since
   * previous update.
   *
   * @param precision
   *          in seconds
   */
  public void setPrecision(final double precision) { _precision = precision; }

  /**
   * Get the variance possible on the release rate. 0.0f = no variance 0.5f = between releaseRate / 2f
   * and 1.5f * releaseRate
   *
   * @return release variance as a percent.
   */
  public double getReleaseVariance() { return _releaseVariance; }
   ...
   */
  public void setReleaseVariance(final double variance) { _releaseVariance = variance; }
   ...
   */
  public boolean isControlFlow() { return _controlFlow; }
   ...
   */
  public void setControlFlow(final boolean regulate) { _controlFlow = regulate; }
   ...
   */
  public boolean isUpdateOnlyInView() { return _updateOnlyInView; }
   ...
   */
  public void setUpdateOnlyInView(final boolean updateOnlyInView) { _updateOnlyInView = updateOnlyInView; }

  /**
   * @return the camera to be used in updateOnlyInView situations. If null, the current
   *         displaySystem's renderer camera is used.
   */
  public Camera getViewCamera() { return _viewCamera; }

  /**
   * @param viewCamera
   *          sets the camera to be used in updateOnlyInView situations. If null, the current
   *          displaySystem's renderer camera is used.
   */
  public void setViewCamera(final Camera viewCamera) { _viewCamera = viewCamera; }
   ...
   * @param iterations
   */
  public void setIterations(final int iterations) { this.iterations = iterations; }
   ...
   * @return ArrayList
   */
  public List<ParticleInfluence> getInfluences() { return influences; }
   ...
   */
  public List<ParticleControllerListener> getListeners() { return listeners; }

  /**
   * Runs the update method of this particle manager X number of times passing .1 seconds for each
   * call. This is used to "warm up" and get the particle manager going.
   *
   * @param iterations
  ...
  public void setSpawnEnabled(final boolean spawnEnabled) { _spawnEnabled = spawnEnabled; }
}
