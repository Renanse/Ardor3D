...
/**
 * Vector3 represents a point or vector in a three dimensional system. This implementation stores
 * its data in double-precision.
 */
public class Vector3 implements Cloneable, Savable, Externalizable, ReadOnlyVector3, Poolable {
  ...
  public double getValue(final int index) {
    return switch (index) {
      ...
      case 1 -> getY();
      case 2 -> getZ();
      default -> throw new IllegalArgumentException("index must be either 0, 1 or 2: " + index);
    };
  }
   ...
   * @param store
   *          the vector to store the result in for return. If null, a new vector object is created
   *          and returned. It IS okay for source and store to be the same object.
   * @return (this.x - source.x, this.y - source.y, this.z - source.z)
   * @throws NullPointerException
   *           if source is null.
   */
   ...
   * the vector is 0, 0, 0) then a new vector (0, 0, 0) is returned.
   *
   * @param store
   *          the vector to store the result in for return. If null, a new vector object is created
   *          and returned.
   ...
   * 0, (ie, if the vector is 0, 0, 0) then no action is taken.
   *
   * @return this vector for chaining
   */
   ...
   * @param store
   *          the vector to store the result in for return. If null, a new vector object is created
   *          and returned. It IS safe for store to be the same as the begin or end vector.
   * @return a new vector as described above.
   * @throws NullPointerException
  ...
  /**
   * @return the magnitude or distance between the origin (0, 0, 0) and the point described by this
   *         vector (x, y, z). Effectively the square root of the value returned by
   *         {@link #lengthSquared()}.
   */
  @Override
  ...
  /**
   * @return the squared magnitude or squared distance between the origin (0, 0, 0) and the point
   *         described by this vector (x, y, z)
   */
  @Override
  ...
  /**
   * @param vec
   * @return this vector, set to the cross product of this vector with the given vector's x, y, z
   *         values
   * @throws NullPointerException
   *           if vec is null.
   */
  ...
  @Override
  public Class<? extends Vector3> getClassTag() { return this.getClass(); }
  ...
  /**
   * @return An instance of Vector3 that is intended for temporary use in calculations and so forth.
  ...
  /**
   * Releases a Vector3 back to be used by a future call to fetchTempInstance. TAKE CARE: this Vector3
