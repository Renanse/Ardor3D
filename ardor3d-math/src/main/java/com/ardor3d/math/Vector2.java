...
/**
 * Vector2 represents a point or vector in a two dimensional system. This implementation stores its
...
public class Vector2 implements Cloneable, Savable, Externalizable, ReadOnlyVector2, Poolable {
  ...
  public double getValue(final int index) {
    return switch (index) {
      case 0 -> getX();
      case 1 -> getY();
      default -> throw new IllegalArgumentException("index must be either 0 or 1: " + index);
    };
  ...
  /**
   * Adds the given values to those of this vector and returns them in store * @param store the vector
   * to store the result in for return. If null, a new vector object is created and returned. .
   *
   * @param x
   * @param y
   * @param store
  ...
  /**
   * Internally modifies the values of this vector by multiplying them each by the values of the given
   * scale.
   *
   * @param scale
   * @return this vector for chaining
  ...
  /**
   * Internally modifies the values of this vector by dividing them each by the values of the given
   * scale.
   *
   * @param scale
   * @return this vector for chaining
   ...
   * the vector is 0, 0) then a new vector (0, 0) is returned.
   *
   * @param store
   *          the vector to store the result in for return. If null, a new vector object is created
   *          and returned.
   ...
   * 0, (ie, if the vector is 0, 0) then no action is taken.
   *
   * @return this vector for chaining
   */
  ...
  /**
   * @return the magnitude of this vector, or the distance between the origin (0, 0) and the point
   *         described by (x, y). Basically sqrt(x^2 + y^2)
   */
  @Override
   ...
   * @param x
   * @param y
   * @return the squared distance between the point described by this vector and the given x, y point.
   *         When comparing the relative distance between two points it is usually sufficient to
   *         compare the squared distances, thus avoiding an expensive square root operation.
   */
  @Override
  ...
  /**
   * @return the angle - in radians [-pi, pi) - represented by this Vector2 as expressed by a
   *         conversion from rectangular coordinates (<code>x</code>,&nbsp;<code>y</code>) to polar
   *         coordinates (r,&nbsp;<i>theta</i>).
   */
  @Override
  public double getPolarAngle() { return -Math.atan2(getY(), getX()); }

  /**
   * @param otherVector
   *          the "destination" unit vector
   * @return the angle (in radians) required to rotate a ray represented by this vector to lie
   *         co-linear to a ray described by the given vector. It is assumed that both this vector and
   *         the given vector are unit vectors (normalized).
   * @throws NullPointerException
   ...
   * @return the minimum angle (in radians) between two vectors. It is assumed that both this vector
   *         and the given vector are unit vectors (normalized).
   * @throws NullPointerException
   *           if otherVector is null.
   */
  @Override
  ...
  @Override
  public Class<? extends Vector2> getClassTag() { return this.getClass(); }
  ...
  /**
   * @return An instance of Vector2 that is intended for temporary use in calculations and so forth.
  ...
  /**
   * Releases a Vector2 back to be used by a future call to fetchTempInstance. TAKE CARE: this Vector2
