 ...
 * </ul>
 *
 * NB: With current improvements in hardware, I don't recommend using these and they will likely be
...
public enum FastMath {
  ...
  public final static double atan(final double x) {
    return Math.abs(x) < 1 ? x / (1 + FastMath._atan_a * x * x)
        : Math.signum(x) * MathUtils.HALF_PI - x / (x * x + FastMath._atan_a);
  }
