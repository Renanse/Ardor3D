...
public class Matrix3 implements Cloneable, Savable, Externalizable, ReadOnlyMatrix3, Poolable {
  ...
  /**
   * Used with equals method to determine if two Matrix3 objects are close enough to be considered
   * equal.
   */
   ...
   */
  @Override
  public boolean isIdentity() { return strictEquals(Matrix3.IDENTITY); }
  ...
  /**
   * Sets this matrix to the rotation indicated by the given angle and axis of rotation. Note: This
   * method creates an object, so use fromAngleNormalAxis when possible, particularly if your axis is
   * already normalized.
   *
   * @param angle
  ...
  /**
   * XXX: Need to redo this again... or at least correct the terms. YRP are arbitrary terms, based on
   * a specific frame of axis.
   *
   * Updates this matrix from the given Euler rotation angles (y,r,p). Note that we are applying in
   * order: roll, pitch, yaw but we've ordered them in x, y, and z for convenience. See:
   ...
   * @param matrix
   * @param store
   *          a matrix to store the result in. if null, a new matrix is created. It is safe for the
   *          given matrix and this parameter to be the same object.
   ...
   * @param store
   *          a vector to store the result in. If store is null, a new vector is created. Note that it
   *          IS safe for vec and store to be the same object.
  ...
  /**
   * Modifies this matrix to equal the rotation required to point the z-axis at 'direction' and the
   * y-axis to 'up'.
   *
   * @param direction
   *          where to 'look' at
  ...
  @Override
  public Class<? extends Matrix3> getClassTag() { return this.getClass(); }
  ...
  /**
   * @return An instance of Matrix3 that is intended for temporary use in calculations and so forth.
  ...
  /**
   * Releases a Matrix3 back to be used by a future call to fetchTempInstance. TAKE CARE: this Matrix3
