 ...
 * by placing a random point in every 1x1x1 unit cube in space and then finding the closest of these
 * points at each eval location.
 *
 */
public class VoroniFunction3D implements Function3D {
   ...
   * @param useDistance
   *          if true, we will add the distance from the closest point to our output, giving us
   *          variation across the cell.
   * @param seed
   *          the random seed value to give to our integer random function.
   */
  public VoroniFunction3D(final double frequency, final double displacement, final boolean useDistance,
    final int seed) {
  ...
  public double eval(final double x, final double y, final double z) {
    ...
    return value + (_displacement * point(MathUtils.floor(minPoint.getXf()), MathUtils.floor(minPoint.getYf()),
  ...
  public boolean isUseDistance() { return _useDistance; }

  public void setUseDistance(final boolean useDistance) { _useDistance = useDistance; }

  public double getDisplacement() { return _displacement; }

  public void setDisplacement(final double displacement) { _displacement = displacement; }

  public int getSeed() { return _seed; }

  public void setSeed(final int seed) { _seed = seed; }
