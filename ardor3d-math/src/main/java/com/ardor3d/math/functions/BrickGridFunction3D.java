...
/**
 * Creates a brick-like pattern in the XY plane. This is not really 3D, since it ignores the z
 * factor.
 *
 * The brick and mortar values returned by this function can (and do by default) have some amount of
 * variation for visual interest. The specified variation represents a maximum -- actual variation
 * is determined randomly within this maximum variation. Since actual variation is determined
 * randomly, the brick and mortar values are likely to change a bit for each instance of this class.
 * In other words, if you do not want the variation to change at all, you should instantiate this
 * class once and use the same object each time.
 */
