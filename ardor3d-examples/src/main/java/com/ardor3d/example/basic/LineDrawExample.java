...
@Purpose(htmlDescriptionKey = "com.ardor3d.example.basic.LineDrawExample", //
...
public class LineDrawExample extends ExampleBase {
    ...
    protected void registerInputTriggers() {
        ...
        _logicalLayer.registerTrigger(
                new InputTrigger(new KeyPressedCondition(Key.LEFT_BRACKET), (source, inputStates, tpf) -> {
                    line.setLineWidth(Math.max(0.5f, line.getLineWidth() - 0.5f));
                    recreateInstructions();
                }));

        _logicalLayer.registerTrigger(
                new InputTrigger(new KeyPressedCondition(Key.RIGHT_BRACKET), (source, inputStates, tpf) -> {
                    line.setLineWidth(Math.min(99f, line.getLineWidth() + 0.5f));
                    recreateInstructions();
                }));

        _logicalLayer
                .registerTrigger(new InputTrigger(new KeyPressedCondition(Key.MINUS), (source, inputStates, tpf) -> {
                    line.setMiterLimit(Math.max(-1f, line.getMiterLimit() - 0.1f));
                    recreateInstructions();
                }));

        _logicalLayer
                .registerTrigger(new InputTrigger(new KeyPressedCondition(Key.EQUAL), (source, inputStates, tpf) -> {
        ...
        _logicalLayer
                .registerTrigger(new InputTrigger(new KeyPressedCondition(Key.THREE), (source, inputStates, tpf) -> {
                    textured = true;
                    textureIndex = 1;
                    updateLineMaterial();
                    recreateInstructions();
