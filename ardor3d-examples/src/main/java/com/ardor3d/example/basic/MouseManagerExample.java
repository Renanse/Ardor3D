...
/**
 * A demonstration of the MouseManager class, which is used to control properties (e.g. cursor,
 * location) of the native mouse.
 */
@Purpose(
    htmlDescriptionKey = "com.ardor3d.example.basic.MouseManagerExample", //
...
public class MouseManagerExample extends ExampleBase {
  ...
  protected void initExample() {
    ...
    try {
      ...
      _logicalLayer.registerTrigger(new InputTrigger(new KeyPressedCondition(Key.H), (source, inputState, tpf) -> {
        if (useCursorOne) {
      ...
      _logicalLayer.registerTrigger(new InputTrigger(new KeyPressedCondition(Key.J),
          (source, inputState, tpf) -> _mouseManager.setCursor(MouseCursor.SYSTEM_DEFAULT)));
      _logicalLayer.registerTrigger(new InputTrigger(new KeyPressedCondition(Key.K), (source, inputState, tpf) -> {
