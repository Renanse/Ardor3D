...
public final class PropertiesDialog extends JDialog {
  ...
  /**
   * Constructor for the <code>PropertiesDialog</code>. Creates a properties dialog initialized for
   * the primary display.
   *
   * @param source
   *          the <code>GameSettings</code> object to use for working with the properties file.
   * @param imageFile
   *          the image file to use as the title of the dialog; <code>null</code> will result in to
  ...
  /**
   * <code>verifyAndSaveCurrentSelection</code> first verifies that the display mode is valid for this
   * system, and then saves the current selection as a properties.cfg file.
   *
  ...
  /**
   * <code>setUpChooser</code> retrieves all available display modes and places them in a
   * <code>JComboBox</code>. The resolution specified by GameSettings is used as the default value.
   *
  ...
  /**
   * <code>updateDisplayChoices</code> updates the available color depth and display frequency options
   * to match the currently selected resolution.
   */
  ...
  /**
   * <code>updateResolutionChoices</code> updates the available resolutions list to match the
   * currently selected window mode (fullscreen or windowed). It then sets up a list of standard
   * options (if windowed) or calls <code>updateDisplayChoices</code> (if fullscreen).
   */
  ...
  private static String[] getDepths(final String resolution, final DisplayMode[] modes) {
    final Set<String> depths = new TreeSet<>((o1, o2) -> -o1.compareTo(o2));
    for (int i = 0; i < modes.length; i++) {
  ...
  /**
   * Utility class for sorting <code>DisplayMode</code>s. Sorts by resolution, then bit depth, and
   * then finally refresh rate.
   */
  ...
  private static class ModesRetriever implements Runnable {
    ...
    public void run() {
      try {
        ...
        logger.logp(Level.SEVERE, this.getClass().toString(), "PropertiesDialog(GameSettings, URL)", "Exception", e);
        return;
      }
      ready = true;
    }
