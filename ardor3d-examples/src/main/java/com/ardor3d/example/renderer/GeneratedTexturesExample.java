...
/**
 * Illustrates the GeneratedImageFactory class and math.functions package, which allow for
 * procedural creation of textures.
 */
@Purpose(
    htmlDescriptionKey = "com.ardor3d.example.renderer.GeneratedTexturesExample", //
...
public class GeneratedTexturesExample extends ExampleBase {
  ...
  protected void registerInputTriggers() {
    ...
    _logicalLayer.registerTrigger(new InputTrigger(TriggerConditions.mouseMoved(), (source, inputStates, tpf) -> {
      if (!allowClicks || !zoom) {
        return;
      }
  ...
  private void setWoodGrain(final UIPanel panel) {
    ...
    final Function3D rotatedWood =
        Functions.rotateInput(translatedWood, new Matrix3().fromAngles(MathUtils.DEG_TO_RAD * 6, 0, 0));
  ...
  private void setJade(final UIPanel panel) {
    ...
    final Function3D perturbedBaseSecondaryJade = new TurbulenceFunction3D(rotatedBaseSecondaryJade, 1.0 / 4.0, 4, 4.0);
  ...
  private void applyTexture(final Texture tex, final UIPanel panel) {
    final ImageBackdrop backdrop =
        new ImageBackdrop(new SubTex(tex, 0, 0, tex.getImage().getWidth(), -tex.getImage().getHeight()));
