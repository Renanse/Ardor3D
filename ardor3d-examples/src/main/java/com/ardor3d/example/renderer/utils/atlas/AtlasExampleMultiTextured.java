...
/**
 * Example showing how to use the TexturePacker to create a texture atlas. Also shows the benefits
 * of using it together with the MeshCombiner.
 */
@Purpose(
    htmlDescriptionKey = "com.ardor3d.example.basic.AtlasExampleMultiTextured", //
...
public class AtlasExampleMultiTextured extends ExampleBase {
  ...
  protected void initExample() {
    ...
    _logicalLayer.registerTrigger(
        new InputTrigger(new KeyPressedCondition(Key.F), (source, inputStates, tpf) -> packIntoAtlas(boxNode)));
    ...
    // Combine into one mesh
    _logicalLayer
        .registerTrigger(new InputTrigger(new KeyPressedCondition(Key.R), (source, inputStates, tpf) -> resetBoxes()));
