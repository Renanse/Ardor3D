 ...
 */
@Purpose(
    htmlDescriptionKey = "com.ardor3d.example.ui.SimpleUIExample", //
...
public class SimpleUIExample extends ExampleBase {
  ...
  private UIPanel makeLoginPanel() {
    ...
    btLogin.addActionListener(
        event -> System.out.println("login as user: " + tfName.getText() + " password: " + tfPassword.getText()));
    pLogin.add(lHeader);
    pLogin.add(lName);
    pLogin.add(tfName);
  ...
  private UIPanel makeChatPanel() {
    ...
    final ActionListener actionListener = event -> applyChat(historyArea, chatField, scrollArea);
  ...
  private UIPanel makeWidgetPanel() {
    ...
    slider.addActionListener(event -> lSliderValue.setText(String.valueOf(slider.getValue())));
    ...
    combo.addSelectionListener((component, newValue) -> System.out.println("New combo value: " + newValue));
    ...
    bar.addController(
        (final double time, final UIProgressBar caller) -> caller.setPercentFilled(_timer.getTimeInSeconds() / 15));
    centerPanel.add(bar);
    return panel;
  }
  ...
  private UIPanel makeClockPanel() {
    ...
    clockPanel.addController((time, caller) -> {
