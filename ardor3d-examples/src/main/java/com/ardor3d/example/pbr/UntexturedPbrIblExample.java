 ...
 */
@Purpose(
    htmlDescriptionKey = "com.ardor3d.example.pbr.UntexturedPbrIblExample", //
  ...
  protected void initExample() {
    ...
    for (int row = 0; row < nrRows; ++row) {
      ...
      for (int col = 0; col < nrColumns; ++col) {
        ...
        mesh.setProperty("surface", new PbrSurface(ColorRGBA.WHITE, metallic, roughness, 1.0f));
  ...
  protected void updateExample(final ReadOnlyTimer timer) {
    for (int i = 0; i < _lightCount; i++) {
      _lights[i].setTranslation(((i % 2 == 1) ? -30 : 30) + Math.sin(timer.getTimeInSeconds() * 2) * 15,
          ((i / 2) % 2 == 1) ? -30 : 30, 30f);
      _lights[i].updateWorldTransform(false);
    }
  }
}
