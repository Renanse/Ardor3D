...
public class ExampleRunner extends JFrame {
  ...
  private final static String HEADER = "<html><body><h2 align=\"center\">Ardor3d Examples</h2>"
      ...
      + "<p align=\"center\"><img src=\"" + ResourceLocatorTool.getClassPathResource(ExampleRunner.class,
  ...
  public ExampleRunner() {
    ...
    for (final Action action : new Action[] {firstMatchAction, nextMatchAction, previousMatchAction, runSelectedAction,
        browseSelectedAction}) {
      toolbar.getActionMap().put(action.getValue(Action.NAME), action);
      toolbar.getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW).put((KeyStroke) action.getValue(Action.ACCELERATOR_KEY),
  ...
  private static ImageIcon getIcon(final String name) {
    return new ImageIcon(
        ResourceLocatorTool.getClassPathResource(ExampleRunner.class, "com/ardor3d/example/media/icons/" + name));
  }
  ...
  interface SearchFilter {
    ...
    int updateMatches(final String p);
  }
  ...
  class ClassTreeModel implements TreeModel, SearchFilter {
    ...
    private FileFilter getFileFilter() {
      if (classFileFilter == null) {
        classFileFilter = pathname -> (pathname.isDirectory()
            && (pathname.getName().length() == 0 || pathname.getName().charAt(0) != '.'))
    ...
    protected void fireTreeNodeChanged(final TreePath tp) {
      ...
      final TreeModelEvent ev =
          new TreeModelEvent(this, parentPath, new int[] {getIndexOfChild(parentObj, obj)}, new Object[] {obj});
      for (final TreeModelListener l : listeners.getListeners(TreeModelListener.class)) {
    ...
    protected void fireTreeNodeChanged(final Package pkg) {
      final TreeModelEvent ev =
          new TreeModelEvent(this, new Object[] {root}, new int[] {getIndexOfChild(root, pkg)}, new Object[] {pkg});
      for (final TreeModelListener l : listeners.getListeners(TreeModelListener.class)) {
        l.treeNodesChanged(ev);
      }
    }
    ...
    protected void fireTreeNodeChanged(final Class<?> clazz) {
      ...
      final TreeModelEvent ev = new TreeModelEvent(this, new Object[] {root, pkg},
          new int[] {getIndexOfChild(pkg, clazz)}, new Object[] {clazz});
      for (final TreeModelListener l : listeners.getListeners(TreeModelListener.class)) {
        l.treeNodesChanged(ev);
      }
    }
