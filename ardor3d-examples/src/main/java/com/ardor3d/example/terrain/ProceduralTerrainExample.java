...
public class ProceduralTerrainExample extends ExampleBase {
    ...
    protected void updateExample(final ReadOnlyTimer timer) {
        ...
        if (terrain != null && terrain.getGeometryClipmapShader() != null) {
            terrain.getGeometryClipmapShader().setUniform("lightDir", dLight.getDirection());
        }

        skybox.setTranslation(camera.getLocation());
        lightTime += timer.getTimePerFrame();
        dLight.setDirection(new Vector3(Math.sin(lightTime), -.8, Math.cos(lightTime)).normalizeLocal());
        // if we're picking...
    ...
    protected void initExample() {
        ...
        try {
            ...
            terrain.setPixelShader(new UrlInputSupplier(ResourceLocatorTool.getClassPathResource(Terrain.class,
                    "com/ardor3d/extension/terrain/texturedGeometryClipmapShader_normalMap.frag")));
