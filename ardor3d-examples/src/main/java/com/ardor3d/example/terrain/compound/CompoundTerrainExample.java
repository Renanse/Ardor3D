...
/**
 * Example showing the Geometry Clipmap Terrain system where terrain data is pulled from multiple sources and combined.
 */
@Purpose(htmlDescriptionKey = "com.ardor3d.example.terrain.CompoundTerrainExample", //
...
public class CompoundTerrainExample extends ExampleBase {
    ...
    protected void initExample() {
        ...
        _logicalLayer.registerTrigger(new InputTrigger(new KeyPressedCondition(Key.U),
                (source, inputStates, tpf) -> updateTerrain = !updateTerrain));
        ...
        _logicalLayer.registerTrigger(new InputTrigger(new KeyPressedCondition(Key.R), (source, inputStates,
                tpf) -> terrain.getTextureClipmap().setShowDebug(!terrain.getTextureClipmap().isShowDebug())));
    ...
    private void setupTerrain() {
        try {
            ...
            final CompoundTerrainDataProvider terrainDataProvider = new CompoundTerrainDataProvider(
                    new TerrainConfiguration(7, 128, new Vector3(6, 300, 6), 0, 1.2f, true));
    ...
    private ExampleEntry createPillarEntry(final double offX, final double offY, final double radius,
            final double height, final ICombineFunction combine) {
