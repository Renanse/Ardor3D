...
/**
 * Another particle demonstration, this one showing a smoking rocket following the cursor around the
 * screen.
 */
@Purpose(
    htmlDescriptionKey = "com.ardor3d.example.effect.NewDynamicSmokerExample", //
...
public class NewDynamicSmokerExample extends ExampleBase {
  ...
  protected void registerInputTriggers() {
    ...
    _logicalLayer.registerTrigger(new InputTrigger(new MouseMovedCondition(), (source, inputStates, tpf) -> {
  ...
  private void updateRocket(double tpf) {
    ...
    final Vector3 dirTowardsMouse =
        new Vector3(worldStore).subtractLocal(rocketEntityNode.getWorldTranslation()).normalizeLocal();
  ...
  private void addUI() {
    ...
    turnSlider.addActionListener(event -> {
    ...
    propelSlider.addActionListener(event -> {
    ...
    ageSlider.addActionListener(event -> {
    ...
    twoDimCheck.addActionListener(event -> stayIn2DPlane = twoDimCheck.isSelected());
    ...
    worldCoordsCheck.addActionListener(event -> {
