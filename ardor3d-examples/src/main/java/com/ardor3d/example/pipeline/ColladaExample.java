...
/**
 * Illustrates loading a model from Collada. If the model also contains an animation, the animation
 * is played as well.
 */
@Purpose(
    htmlDescriptionKey = "com.ardor3d.example.pipeline.ColladaExample", //
...
public class ColladaExample extends ExampleBase {
  ...
  private void createHUD() {
    ...
    loadSceneButton.addActionListener(event -> {
    ...
    skinCheck.setLayoutData(new AnchorLayoutData(Alignment.TOP_LEFT, loadSceneButton, Alignment.BOTTOM_LEFT, 0, -5));
    skinCheck.setSelected(true);
    skinCheck.addActionListener(
        event -> colladaNode.getSceneHints().setCullHint(skinCheck.isSelected() ? CullHint.Dynamic : CullHint.Always));
