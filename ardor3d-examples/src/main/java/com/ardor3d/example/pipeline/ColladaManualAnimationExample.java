 ...
 */
@Purpose(
    htmlDescriptionKey = "com.ardor3d.example.pipeline.ColladaManualAnimationExample", //
...
public class ColladaManualAnimationExample extends ExampleBase {
  ...
  private void targetJoint(final SkeletonPose pose, final int jointIndex, final ReadOnlyVector3 bindPoseDirection,
    ...
    // neckBindGlobalTransform is the neck bone -> model space transform. essentially, it is the world
    // transform of
    ...
    // Get a vector representing forward direction in neck space, use inverse to take from world -> neck
    // space.
    ...
    // Calculate a rotation to go from one direction to the other and set that rotation on a blank
    // transform.
    ...
    // now remove the global/world transform of the neck's parent bone, leaving us with just the local
    // transform of
    // neck + rotation.
  ...
  private void positionCamera() {
    ...
    if (bounding != null) {
      ...
      _canvas.getCanvasRenderer().getCamera().setFrustumPerspective(50.0,
