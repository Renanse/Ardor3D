 ...
 */
@Purpose(
    htmlDescriptionKey = "com.ardor3d.example.pipeline.AnimationCopyExample", //
...
public class AnimationCopyExample extends ExampleBase {
  ...
  private void createHUD() {
    ...
    headCheck
        .addActionListener(event -> manager.getValuesStore().put("head_blend", headCheck.isSelected() ? 1.0 : 0.0));
    panel.add(headCheck);

    final UICheckBox gpuSkinningCheck = new UICheckBox("Use GPU skinning");
    gpuSkinningCheck.setLayoutData(new AnchorLayoutData(Alignment.TOP_LEFT, headCheck, Alignment.BOTTOM_LEFT, 0, -5));
  ...
  private void createAnimation() {
    ...
    try {
      final ResourceSource layersFile = new URLResourceSource(ResourceLocatorTool
          .getClassPathResource(AnimationCopyExample.class, "com/ardor3d/example/pipeline/AnimationCopyExample.js"));
    ...
    final ManagedTransformSource headSource =
        (ManagedTransformSource) manager.findAnimationLayer("head").getSteadyState("head_rotate").getSourceTree();
