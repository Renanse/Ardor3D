...
public abstract class PickingUtil {
  /**
   * Finds a pick using the given ray starting at the scenegraph given as spatial. Results are stored
   * in the given results value.
   * 
  ...
  public static void findPick(final Spatial spatial, final Ray3 ray, final PickResults results,
    ...
    if (spatial == null || !spatial.getSceneHints().isPickingHintEnabled(PickingHint.Pickable)
        || (ignoreCulled && spatial.getSceneHints().getCullHint() == CullHint.Always) || spatial.getWorldBound() == null
        || !spatial.getWorldBound().intersects(ray)) {
      return;
    }
   ...
   */
  public static void findPrimitiveCollision(final Mesh testMesh, final Mesh toCheck, final List<PrimitiveKey> testIndex,
