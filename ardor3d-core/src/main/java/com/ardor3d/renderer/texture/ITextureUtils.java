...
public interface ITextureUtils {
  ...
  /**
   * Explicitly remove this Texture from the graphics card. Note, the texture is only removed for the
   * current context. If the texture is used in other contexts, those uses are not touched. If the
   * texture is not used in this context, this is a no-op.
   *
   * @param texture
   ...
   *
   * @param destination
   *          the texture to update. Should already have been sent to the card (have a valid texture
   *          id.)
   * @param dstOffsetX
  ...
  void updateTexture3DSubImage(Texture3D destination, int dstOffsetX, int dstOffsetY, int dstOffsetZ, int dstWidth,
      int dstHeight, int dstDepth, ByteBuffer source, int srcOffsetX, int srcOffsetY, int srcOffsetZ, int srcTotalWidth,
      int srcTotalHeight);
