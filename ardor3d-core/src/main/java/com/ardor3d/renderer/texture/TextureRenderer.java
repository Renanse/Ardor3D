...
/**
 * <code>TextureRenderer</code> defines an abstract class that handles rendering a scene to a buffer
 * and copying it to a texture. Creation of this object is usually handled via a
 * TextureRendererFactory. Texture2D and TextureCubeMap are currently supported as render targets.
 */
public interface TextureRenderer {
   ...
   * @param tex
   *          the Texture to render to. This should be a Texture2D or TextureCubeMap. If the latter,
  ...
  /**
   * NOTE: If more than one texture is given, copy-texture is used regardless of card capabilities to
   * decrease render time.
   *
   * @param renderable
   *          the renderable to render.
   * @param texs
   *          a list of Textures to render to. These should be of type Texture2D or TextureCubeMap. If
   *          the latter, its currentRTTFace will determine which cube face is drawn to.
   * @param clear
   *          which buffers to clear before rendering, if any.
   * @see Renderer#BUFFER_COLOR et al
   */
   ...
   * that is not set via typical rendering operations.
   *
   * @param c
  ...
  /**
   * <code>setupTexture</code> initializes a Texture object for use with TextureRenderer. Generates a
   * valid gl texture id for this texture and sets up data storage for it. The texture will be equal
   * to the texture renderer's size.
   *
   * Note that the texture renderer's size is not necessarily what is specified in the constructor.
   *
   * @param tex
   *          The texture to setup for use in Texture Rendering. This should be of type Texture2D or
   *          TextureCubeMap.
   */
  void setupTexture(Texture tex);

  /**
   * <code>copyToTexture</code> copies the current frame buffer contents to the given Texture. What is
   * copied is based on the rttFormat of the texture object when it was setup. Note that the contents
   * are copied with no scaling applied, so the texture must be big enough such that xoffset + width
   * <= texture's width and yoffset + height <= texture's height.
   *
   * @param tex
   *          The Texture to copy into. This should be a Texture2D or TextureCubeMap. If the latter,
   *          its currentRTTFace will determine which cube face is drawn to.
   * @param x
   ...
   *          minification levels that support mipmapping.
   */
  void setEnableMipGeneration(boolean enable);

  /**
   * @return true (the default) if we automatically generate mipmaps for textures that have
   *         minification levels that support mipmapping.
   */
  boolean isEnableMipGeneration();

  /**
   * @return the requested depth buffer bits of this Texture Renderer - may not be the *actual* bits
   *         used. Default is 0, which means to allow OpenGL to choose.
   */
  int getDepthBits();
  ...
  /**
   * Enforce a particular state whenever this texture renderer is used. In other words, the given
   * state will override any state of the same type set on a scene object rendered with this texture
   * renderer.
   *
   * @param state
   *          state to enforce
