...
public class SimpleBloomEffect extends RenderEffect {
  ...
  protected void initTargets(final EffectManager manager) {
    ...
    final RenderTarget_Texture2D main =
        new RenderTarget_Texture2D(downsampledWidth, downsampledHeight, manager.getOutputFormat());
    main.getTexture().setWrap(WrapMode.EdgeClamp);
    manager.getRenderTargetMap().put(RT_MAIN, main);

    final RenderTarget_Texture2D secondary =
        new RenderTarget_Texture2D(downsampledWidth, downsampledHeight, manager.getOutputFormat());
    secondary.getTexture().setWrap(WrapMode.EdgeClamp);
    manager.getRenderTargetMap().put(RT_SECONDARY, secondary);
  }

  public void setExposureIntensity(final float value) { _exposureIntensity = value; }

  public float getExposureIntensity() { return _exposureIntensity; }

  public void setExposureCutoff(final float value) { _exposureCutoff = value; }

  public float getExposureCutoff() { return _exposureCutoff; }

  public float getBlendExposure() { return _blendExposure; }

  public void setBlendExposure(final float blendExposure) { _blendExposure = blendExposure; }

  public void setDownsampleRatio(final float downsampleRatio) { _downsampleRatio = downsampleRatio; }

  public float getDownsampleRatio() { return _downsampleRatio; }

  public List<Spatial> getBloomItems() { return _bloomItems; }
}
