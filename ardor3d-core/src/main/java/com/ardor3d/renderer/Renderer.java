...
/**
 * <code>Renderer</code> defines an interface for classes that handle displaying of graphics data to
 * a render context.
 *
...
public interface Renderer {
   ...
   *          the buffers to clear.
   * @param strict
   *          if true, we'll limit the clearing to just the viewport area specified by the current
   *          camera.
   * @see #BUFFER_COLOR
   * @see #BUFFER_DEPTH
   * @see #BUFFER_ACCUMULATION
  ...
  /**
   * <code>flushFrame</code> handles rendering any items still remaining in the render buckets and
   * optionally swaps the back buffer with the currently displayed buffer.
   *
   * @param doSwap
   *          if true, will ask the underlying implementation to blit the back buffer contents to the
   *          display buffer. Usually this will be true, unless you are in a windowing toolkit that
   *          handles doing this for you.
   */
  ...
  /**
   * <code>grabScreenContents</code> reads a block of data as bytes from the current framebuffer. The
   * format determines how many bytes per pixel are read and thus how big the buffer must be that you
   * pass in.
   *
   * @param store
   ...
   * renderer hands off management of the scene to spatial for it to determine when a
  ...
  /**
   * <code>flush</code> tells the graphics hardware to send through all currently waiting commands in
   * the buffer.
   */
  void flushGraphics();

  /**
   * <code>finish</code> is similar to flush, however it blocks until all waiting hardware graphics
  ...
  /**
   * Set our GL_POINT size. Either point size is controlled by the shader, or if that is false, a
   * single point size is used as given.
   *
   ...
   * @return the actual state we applied - could be different from state if another state is enforced,
   *         or if state was null.
   */
   ...
   * @return the appropriate render state for the current context for the current type. This is the
   *         enforced state if one exists or the given current state if not null. Otherwise, the
   *         Renderer's default state is returned.
   */
