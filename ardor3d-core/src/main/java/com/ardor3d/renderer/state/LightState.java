...
/**
 * <code>LightState</code> maintains a collection of lights up to the set number of maximum lights
 * allowed. Any subclass of <code>Light</code> can be added to the light state. Each light is
 * processed and used to modify the color of the scene.
 */
public class LightState extends RenderState {
  ...
  @Override
  public StateType getType() { return StateType.Light; }

  /**
   *
   * <code>attach</code> places a light in the queue to be processed. If there are already eight
   * lights placed in the queue, the light is ignored and false is returned. Otherwise, true is
   * returned to indicate success.
   *
   * @param light
   *          the light to add to the queue.
   * @return true if the light was added successfully, false if there are already eight lights in the
   *         queue.
   */
  ...
  /**
   *
   * <code>get</code> retrieves a particular light defined by an index. If there exists no light at a
   * particular index, null is returned.
   *
   * @param i
