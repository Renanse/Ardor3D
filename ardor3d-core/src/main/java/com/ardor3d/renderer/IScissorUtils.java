...
public interface IScissorUtils {
  ...
  /**
   * Add the given rectangle to the clip stack, clipping the rendering area by the given rectangle
   * intersected with any existing scissor entries. Enable clipping if this is the first rectangle in
   * the stack.
   *
   * @param rectangle
   */
  ...
  /**
   * Push a clip onto the stack indicating that future clips should not intersect with clips prior to
   * this one. Basically this allows you to ignore prior clips for nested drawing. Make sure you pop
   * this using {@link #popClip()}.
   */
  void pushEmptyClip();
