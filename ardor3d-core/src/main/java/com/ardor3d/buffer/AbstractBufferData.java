...
public abstract class AbstractBufferData<T extends Buffer> implements Savable {
   ...
   */
  public int getValuesPerTuple() { return _valuesPerTuple; }
   ...
   *          the buffer to set
   */
  public void setBuffer(final T buffer) { _buffer = buffer; }

  /**
   * @param context
   *          the OpenGL context to get our id for.
   * @return the buffer id of a buffer in the given context. If the buffer is not found in the given
   ...
   * @return false if the buffer is dirty for the given context or we don't have the given object in
   *         memory.
   */
  ...
  public VBOAccessMode getVboAccessMode() { return _vboAccessMode; }
  ...
  /**
   * Clean any VBO ids from the hardware, using the given utility object to do the work immediately,
