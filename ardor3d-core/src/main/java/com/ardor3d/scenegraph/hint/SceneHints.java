...
public class SceneHints implements Savable {
  ...
  /**
   * A flag indicating if scene culling should be done on this object by inheritance, dynamically,
   * never, or always.
   */
  protected CullHint _cullHint = CullHint.Inherit;

  /**
   * Flag signaling how spatial properties should be inherited for this node. By default set to
   * INHERIT.
   */
  ...
  /**
   * Field for setting whether the Spatial is enabled for Picking or Collision. Both are enabled by
   * default.
   */
  ...
  /**
   * Returns the normals mode. If the mode is set to inherit, then we get its normals mode from the
   * given source's hintable parent. If no parent, we'll default to NormalizeIfScaled.
   *
   ...
   */
  public NormalsMode getLocalNormalsMode() { return _normalsMode; }

  /**
   * @param mode
   *          the new normals mode to set on this SceneHints
   */
  public void setNormalsMode(final NormalsMode mode) { _normalsMode = mode; }
   ...
   */
  public CullHint getLocalCullHint() { return _cullHint; }

  /**
   * <code>setCullHint</code> sets how scene culling should work on this spatial during drawing.
   * CullHint.Dynamic: Determine via the defined Camera planes whether or not this Spatial should be
   * culled. CullHint.Always: Always throw away this object and any children during draw commands.
   * CullHint.Never: Never throw away this object (always draw it) CullHint.Inherit: Look for a
   * non-inherit parent and use its cull mode. NOTE: You must set this AFTER attaching to a parent or
   ...
   */
  public void setCullHint(final CullHint hint) { _cullHint = hint; }

  /**
   * Returns this spatial's texture combine mode. If the mode is set to inherit, then the spatial gets
   ...
   */
  public TextureCombineMode getLocalTextureCombineMode() { return _textureCombineMode; }
   ...
   */
  public PropertyMode getLocalPropertyMode() { return _propertyMode; }
  ...
  /**
   * Get the render bucket type used to determine which "phase" of the rendering process this Spatial
   * will rendered in.
   * <p>
   * This method returns the effective bucket type that is used for rendering. If the type is set to
   * {@link com.ardor3d.renderer.queue.RenderBucketType#Inherit Inherit} then the bucket type from the
   * spatial's parent will be used during rendering. If no parent, then
   ...
   * @see com.ardor3d.renderer.queue.RenderBucketType
   */
  public RenderBucketType getLocalRenderBucketType() { return _renderBucketType; }

  /**
   * Set the render bucket type used to determine which "phase" of the rendering process this Spatial
   * will rendered in.
   *
   ...
   *         "further into the screen" and lower values are "closer". Or in other words, if you draw
   *         two quads, one with a zorder of 1 and the other with a zorder of 2, the quad with zorder
   *         of 2 will be "under" the other quad.
   */
  public int getOrthoOrder() { return _orthoOrder; }

  /**
   * @param orthoOrder
   */
  public void setOrthoOrder(final int orthoOrder) { _orthoOrder = orthoOrder; }

  /**
   * Returns the transparency rendering type. If the mode is set to inherit, then we get its type from
   * the given source's hintable parent. If no parent, we'll default to OnePass.
   *
   ...
   */
  public TransparencyType getLocalTransparencyType() { return _transpType; }
  ...
  @Override
  public Class<? extends SceneHints> getClassTag() { return this.getClass(); }
