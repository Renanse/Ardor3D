 ...
 * approximation is an octahedron; each level of refinement increases the number of polygons by a
 * factor of 4.
 * <p/>
 * Shared vertices are not retained, so numerical errors may produce cracks between polygons at high
 * subdivision levels.
 * <p/>
...
public class GeoSphere extends Mesh {
  ...
  public boolean isUsingIcosahedron() { return _usingIcosahedron; }
  ...
  private void updateGeometry() {
    ...
    _meshData.setTextureCoords(new FloatBufferData(
        BufferUtils.createVector2Buffer(textureCoords != null ? textureCoords.getBuffer() : null, verts), 2), 0);

    int pos = 0;

    Triangle[] old;
    if (_usingIcosahedron) {
      final int[] indices = new int[] {pos + 0, pos + 1, pos + 2, pos + 0, pos + 2, pos + 3, pos + 0, pos + 3, pos + 4,
          pos + 0, pos + 4, pos + 5, pos + 0, pos + 5, pos + 1, pos + 1, pos + 10, pos + 6, pos + 2, pos + 6, pos + 7,
          pos + 3, pos + 7, pos + 8, pos + 4, pos + 8, pos + 9, pos + 5, pos + 9, pos + 10, pos + 6, pos + 2, pos + 1,
          pos + 7, pos + 3, pos + 2, pos + 8, pos + 4, pos + 3, pos + 9, pos + 5, pos + 4, pos + 10, pos + 1, pos + 5,
          pos + 11, pos + 7, pos + 6, pos + 11, pos + 8, pos + 7, pos + 11, pos + 9, pos + 8, pos + 11, pos + 10,
          pos + 9, pos + 11, pos + 6, pos + 10};
      ...
      final Triangle[] octahedron = new Triangle[] {new Triangle(yplus, zplus, xplus), new Triangle(xmin, zplus, yplus),
          new Triangle(ymin, zplus, xmin), new Triangle(xplus, zplus, ymin), new Triangle(zmin, yplus, xplus),
    ...
    for (int level = 1; level < _maxlevels; level++) {
      ...
      /**
       * Subdivide each polygon in the old approximation and normalize the next points thus generated to
       * lie on the surface of the unit sphere. Each input triangle with vertBuf labeled [0,1,2] as shown
      ...
      for (final int aPt : triangle.pt) {
        ...
        if (point.getX() > 0 && point.getY() == 0) {
          ...
          final double yCenter = (_meshData.getVertexBuffer().get(triangle.pt[0] * 3 + 1)
              + _meshData.getVertexBuffer().get(triangle.pt[1] * 3 + 1)
              + _meshData.getVertexBuffer().get(triangle.pt[2] * 3 + 1)) / 3.0;
          if (yCenter > 0.0) {
            put(point, true);
