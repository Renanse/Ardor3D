...
public class Tube extends Mesh {
  ...
  /**
   * 
   * @return true if the normals are inverted to point into the torus so that the face is oriented for
   *         a viewer inside the torus. false (the default) for exterior viewing.
   */
   ...
   * 
   * @param viewInside
   *          if true, the normals are inverted to point into the torus so that the face is oriented
   *          for a viewer inside the torus. Default is false (for outside viewing)
   */
