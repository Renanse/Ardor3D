...
public class StripBox extends Mesh implements Cloneable {
  ...
  /**
   * instantiates a new <code>StripBox</code> object. All information must be applies later. For
   * internal usage only
   */
  public StripBox() {
    super("temp");
  ...
  /**
   * Constructor instantiates a new <code>StripBox</code> object. Center and vertex information must
   * be supplied later.
   *
   * @param name
  ...
  /**
   * Constructor instantiates a new <code>StripBox</code> object. The minimum and maximum point are
   * provided. These two points define the shape and size of the box, but not it's orientation or
   * position. You should use the <code>setTranslation</code> and <code>setLocalRotation</code> for
   * those attributes.
   *
   * @param name
   *          the name of the scene element. This is required for identification and comparison
  ...
  /**
   * Constructs a new box. The box has the given center and extends in the x, y, and z out from the
   * center (+ and -) by the given amounts. So, for example, a box with extent of .5 would be the unit
   * cube.
   *
   * @param name
   *          Name of the box.
  ...
  /**
   * Changes the data of the box so that the two opposite corners are minPoint and maxPoint. The other
   * corners are created from those two poitns. If update buffers is flagged as true, the
   * vertex/normal/texture/color/index buffers are updated when the data is changed.
   *
   * @param minPoint
  ...
  /**
   * Changes the data of the box so that its center is <code>center</code> and it extends in the x, y,
   * and z directions by the given extent. Note that the actual sides will be 2x the given extent
   * values because the box extends in + & - from the center for each extent.
   *
   * @param center
  ...
  /**
   *
   * <code>setVertexData</code> sets the vertex positions that define the box. These eight points are
   * determined from the minimum and maximum point.
   *
   */
