...
/**
 * InterpolationController class is an abstract class containing all the stuff common to controllers
 * interpolating things.
 * <p>
 * Implementation note: This class is comprised of quite a few protected methods, this is mainly to
 * allow maximum flexibility for overriding classes.
 * </p>
 *
 * @param <C>
...
public abstract class InterpolationController<C, T extends Spatial> extends ComplexSpatialController<T> {
   ...
   * <p>
   * It will only update the given object if this controller is {@link #isActive() active}, caller
   * isn't <code>null</code> and it's {@link #getSpeed() speed} is greater than zero.
   * </p>
   *
   * @param time
   ...
   */
  protected int getMinimumIndex() { return 0; }

  /**
   * @return The maximum allowed index. By default it returns the last index in the
   *         {@link #getControls() control} list.
   */
  protected int getMaximumIndex() { return getControls().size() - 1; }
   ...
   * <p>
   * This is called automatically from {@link #update(double, Spatial)} and shouldn't be called
   * manually. It only really exists as a separate method to allow sub classes maximum flexibility.
   * Also of note is the fact that if this method is overridden then {@link #getControlFrom()} and
   * {@link #getControlTo()} methods will also probably need to be overridden as they rely on this
   * method clamping the index correctly before they get called.
   * </p>
   */
  ...
  /**
   * This method assumes the {@link #getIndex() index} has already been {@link #clampIndex() clamped}
   * correctly.
   *
   ...
   * @see #getIndex()
   */
  protected void setIndex(final int index) { _index = index; }

  /**
   * @return The index of the {@link #getControls() control} to interpolate from.
   * @see #setIndex(int)
   */
  protected int getIndex() { return _index; }
  ...
  /**
   * @param delta
   *          The new distance between the {@link #getControlFrom() from control} and
   *          {@link #getControlTo() to control} , should be between <code>0.0</code> and
   *          <code>1.0</code> (inclusive).
   * @see #getDelta()
   */
  protected void setDelta(final double delta) { _delta = delta; }

  /**
   * @return The distance between the {@link #getControlFrom() from control} and
   *         {@link #getControlTo() to control} , will be between <code>0.0</code> and
   *         <code>1.0</code> (inclusive).
   * @see #setDelta(double)
   */
  protected double getDelta() { return _delta; }

  /**
   * @param cycleForward
   *          <code>true</code> to interpolate the controls forwards (1, 2, 3 ...), <code>false</code>
   *          to interpolate the controls backwards (3, 2, 1 ...)
   * @see #isCycleForward()
   */
  protected void setCycleForward(final boolean cycleForward) { _cycleForward = cycleForward; }

  /**
   * @return <code>true</code> if interpolating the controls forwards (1, 2, 3 ...),
   *         <code>false</code> if interpolating the controls backwards (3, 2, 1 ...)
   * @see #setCycleForward(boolean)
   */
  protected boolean isCycleForward() { return _cycleForward; }

  /**
   * Also {@link #reset() resets} this controller for safety, because changing the repeat type part
   * way through an interpolation can cause problems.
   *
   * @param repeatType
   ...
   *         {@link ComplexSpatialController.RepeatType#CLAMP clamp} and its currently clamped at the
   *         maximum index.
   */
  public boolean isClamped() { return isRepeatTypeClamp() && getIndex() == getMaximumIndex(); }
