...
public class CurveInterpolationController extends Vector3InterpolationController {
  ...
  /**
   * Look up table of arc lengths (used for constant speed when travelling in reverse (repeat type =
   * cycle))
   */
  ...
  protected double incrementDelta(final double by) {
    ...
    /*
     * If constant speed we need to also check we aren't clamped at max index before we call lookup in
     * the arc length table because there would be no point
     */
    if (isConstantSpeed()) {
      _distance += by;
  ...
  public void setCurve(final Curve curve) {
    ...
    if (isConstantSpeed()) {
      LOGGER.warning(
          "Constant speed is set to true, you will need to call generateArcLengths() to avoid errors during update.");
    }
  }
  ...
  /*
   * Overrides to provide a warning about generating arc lengths if constant speed is set to true and
  ...
  public void setConstantSpeed(final boolean constantSpeed) {
    ...
    if (isConstantSpeed() && null == _arcLengths) {
      LOGGER.warning(
          "Constant speed was set to true, you will need to call generateArcLengths() to avoid errors during update.");
    }
  }

  /**
   * Generates the arc lengths, generates the reverse table if the
   * {@link #setRepeatType(com.ardor3d.scenegraph.controller.ComplexSpatialController.RepeatType)
   * repeat type} is set to {@link ComplexSpatialController.RepeatType#CYCLE cycle}
   * 
   * @param step
  ...
  /**
   * Generates arc lengths which are required if you wish to have {@link #setConstantSpeed(boolean)
   * constant speed} interpolation.
   * 
   * @param step
   *          'See Also:' method for more info.
   * @param reverse
   *          <code>true</code> to also generate a reverse look up table. This is only required if you
   *          plan to use the {@link ComplexSpatialController.RepeatType#CYCLE} repeat type.
  ...
  /**
   * Since splines require at least 4 points to interpolate correctly the default maximum value is
   * overridden to 1 less than normal.
   */
  @Override
  protected int getMaximumIndex() { return super.getMaximumIndex() - 1; }

  /**
   * Since splines require at least 4 points to interpolate correctly the default minimum value is
   * overridden to 1 more than normal.
   */
  @Override
  protected int getMinimumIndex() { return super.getMinimumIndex() + 1; }
