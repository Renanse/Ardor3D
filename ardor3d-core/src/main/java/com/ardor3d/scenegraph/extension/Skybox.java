 ...
 * Either attach to a camera node or update on each frame to set this skybox at the camera's
 * position.
 */
public class Skybox extends Node {
  ...
  /**
   * Creates a new skybox. The size of the skybox and name is specified here. By default, no textures
   * are set.
   *
   * @param name
  ...
  /**
   * Set the texture to be displayed on the given face of the skybox. Replaces any existing texture on
   * that face.
   *
   * @param face
   *          the face to set
  ...
  /**
   * Set the texture to be displayed on the given side of the skybox. Only replaces the texture at the
   * index specified by textureUnit.
   *
   * @param face
   *          the face to set
   * @param texture
  ...
  public void setTexture(final Face face, final Texture texture, final int textureUnit) {
    ...
    TextureState ts = (TextureState) _skyboxQuads[face.ordinal()].getLocalRenderState(RenderState.StateType.Texture);
    if (ts == null) {
      ts = new TextureState();
    }
  ...
  public void initialize() {
    ...
    _skyboxQuads[Face.Up.ordinal()].setRotation(new Matrix3().fromAngles(Math.toRadians(90), Math.toRadians(270), 0));
  ...
  public void preloadTexture(final Face face, final Renderer r) {
    final TextureState ts =
        (TextureState) _skyboxQuads[face.ordinal()].getLocalRenderState(RenderState.StateType.Texture);
    if (ts != null) {
  ...
  /**
   * Force all of the textures to load. This prevents pauses later during the application as you pan
   * around the world.
   */
