...
/**
 * <p>
 * <code>BillboardNode</code> defines a node that will attempt to orient itself in relation to the
 * current camera. The way it does this depends on the alignment type set via
 * {@link #setAlignment(BillboardAlignment)}.
 * </p>
 * <p>
 * <code>BillboardNode</code> is often a useful way fake complex distant geometry using a single
 * quad that has an image applied to it. This quad, with the texture, will appear to be a full model
 * at great distances, and save on rendering and memory.
 * </p>
 * <p>
...
public class BillboardNode extends Node {
  ...
  public enum BillboardAlignment {
    /**
     * Do not change the node's rotation. Useful for situations where you can not remove a billboard
     * node from a hierarchy, but do not want the rotation to change.
     */
    None,
  ...
  /**
   * Normally a billboard triggers transform and bounds updates on its children. Setting this to false
   * will only trigger dirty flags for transform, potentially saving some expensive bounds
   * calculations if those are deemed unnecessary (for example, when using sphere bounds and quad
   * billboards. Setting to false may cause odd culling behavior.
   *
   * @param doUpdate
   *          true (the default) if we should request bounds updates for children, false if not.
   */
  public void setUpdateBounds(final boolean doUpdate) { _updateBounds = doUpdate; }

  /**
   * @return true if bounds are dirtied for children each frame.
   * @see #setUpdateBounds(boolean)
   */
  public boolean isUpdateBounds() { return _updateBounds; }

  public void setLocalRotation(final Matrix3 rot) { _localRot = rot; }

  public ReadOnlyMatrix3 getLocalRotation() { return _localRot; }
   ...
   */
  public BillboardAlignment getAlignment() { return _alignment; }

  /**
   * Sets the type of rotation this BillboardNode will have. The alignment can be ScreenAligned,
   * CameraAligned, AxialY or AxialZ. Invalid alignments will assume no billboard rotation.
   */
