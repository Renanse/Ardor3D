 ...
 * a particular piece of geometry. This sphere defines a radius and a center. <br>
 * <br>
...
public class BoundingSphere extends BoundingVolume {
  ...
  /**
   * <code>computeFromPoints</code> creates a new Bounding Sphere from a given set of points. It uses
   * the <code>calcWelzl</code> method as default.
   * 
   * @param points
  ...
  private void setSphere(final Vector3 O, final Vector3 A, final Vector3 B, final Vector3 C) {
    ...
    if (Denominator == 0) {
      ...
      final Vector3 o =
          a.cross(b, null).multiplyLocal(c.lengthSquared()).addLocal(c.cross(a, null).multiplyLocal(b.lengthSquared()))
  ...
  private void setSphere(final Vector3 O, final Vector3 A) {
    setRadius(Math.sqrt(((A.getX() - O.getX()) * (A.getX() - O.getX()) + (A.getY() - O.getY()) * (A.getY() - O.getY())
  ...
  private BoundingSphere mergeLocalOBB(final OrientedBoundingBox volume) {
    // check for infinite bounds to prevent NaN values... is so, return infinite bounds with center at
    // origin
  ...
  public IntersectionRecord intersectsWhere(final ReadOnlyRay3 ray) {
    ...
    if (a <= 0.0) {
      ...
      final Vector3[] points =
          new Vector3[] {ray.getDirection().multiply(distances[0], new Vector3()).addLocal(ray.getOrigin())};
  ...
  @Override
  public double getVolume() { return 4 * MathUtils.ONE_THIRD * MathUtils.PI * getRadius() * getRadius() * getRadius(); }
}
