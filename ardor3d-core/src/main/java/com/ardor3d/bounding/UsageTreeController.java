...
/**
 * UsageTreeController defines a CollisionTreeController implementation that removes cache elements
 * based on the frequency of usage. By default, and implementation in the CollisionTreeManager, the
 * cache's key set will be ordered with the first element being the oldest used. Therefore,
 * UsageTreeController simply removes elements from the cache starting at the first key and working
 * up until the desired size is reached or we run out of elements.
 */
public class UsageTreeController implements CollisionTreeController {
   ...
   * It removes elements from the keyset as they are ordered.
   * 
   * @param cache
   *          the cache to clean.
   * @param protectedList
