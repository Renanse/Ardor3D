...
/**
 * CollisionTree defines a well balanced red black tree used for triangle accurate collision
 * detection. The CollisionTree supports three types: Oriented Bounding Box, Axis-Aligned Bounding
 * Box and Sphere. The tree is composed of a hierarchy of nodes, all but leaf nodes have two
 * children, a left and a right, where the children contain half of the triangles of the parent.
 * This "half split" is executed down the tree until the node is maintaining a set maximum of
 * triangles. This node is called the leaf node. Intersection checks are handled as follows:<br>
 * 1. The bounds of the node is checked for intersection. If no intersection occurs here, no further
 * processing is needed, the children (nodes or triangles) do not intersect.<br>
 * 2a. If an intersection occurs and we have children left/right nodes, pass the intersection
 * information to the children.<br>
 * 2b. If an intersection occurs and we are a leaf node, pass each triangle individually for
 * intersection checking.<br>
 * <br>
 * Optionally, during creation of the collision tree, sorting can be applied. Sorting will attempt
 * to optimize the order of the triangles in such a way as to best split for left and right
 * sub-trees. This function can lead to faster intersection tests, but increases the creation time
 * for the tree. The number of triangles a leaf node is responsible for is defined in
 * CollisionTreeManager. It is actually recommended to allow CollisionTreeManager to maintain the
...
public class CollisionTree implements Serializable {
  ...
  /**
   * the list of primitives indices that compose the tree. This list contains all the triangles of the
   * mesh and is shared between all nodes of this tree. Stored here to allow for sorting.
   */
  ...
  public void createTree(final int section, final int start, final int end, final boolean doSort) {
    ...
    // if doSort is set we need to attempt to optimize the referenced primitives. optimizing the sorting
    // of the
  ...
  /**
   * Tests if the world bounds of the node at this level intersects a provided bounding volume. If an
   * intersection occurs, true is returned, otherwise false is returned. If the provided volume is
  ...
  /**
   * Determines if this Collision Tree intersects the given CollisionTree. If a collision occurs, true
   * is returned, otherwise false is returned. If the provided collisionTree is invalid, false is
   * returned.
   * 
   * @param collisionTree
   *          The Tree to test.
   ...
   * Determines if this Collision Tree intersects the given CollisionTree. If a collision occurs, true
   * is returned, otherwise false is returned. If the provided collisionTree is invalid, false is
   * returned. All collisions that occur are stored in lists as an integer index into the mesh's
   * triangle buffer. where aList is the primitives for this mesh and bList is the primitives for the
   * test tree.
   * 
   * @param collisionTree
   *          The Tree to test.
   ...
   * are stored in a provided list as primitive index values. The ray is assumed to have a normalized
   * direction for accurate calculations.
   * 
   * @param ray
   *          the ray to test for intersections.
   * @param store
   *          a list to fill with the index values of the primitive hit. if null, a new List is
   *          created.
   * @return the list.
   */
  public List<PrimitiveKey> intersect(final Ray3 ray, final List<PrimitiveKey> store) {
    ...
    if (_right != null) {
      ...
      // This is a leaf node. We can therefore check each primitive this node contains. If an intersection
      // occurs,
   ...
   */
  public BoundingVolume getBounds() { return _bounds; }
   ...
   */
  public BoundingVolume getWorldBounds() { return _worldBounds; }
  ...
  /**
   * sortPrimitives attempts to optimize the ordering of the subsection of the array of primitives
   * this node is responsible for. The sorting is based on the most efficient method along an axis.
   * Using the TreeComparator and quick sort, the subsection of the array is sorted.
   */
   ...
   */
  private Mesh getMesh() { return _mesh.get(); }
