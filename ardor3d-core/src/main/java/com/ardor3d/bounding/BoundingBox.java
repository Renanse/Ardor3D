...
/**
 * <code>BoundingBox</code> defines an axis-aligned cube that defines a container for a group of
 * vertices of a particular piece of geometry. This box defines a center and extents from that
 * center along the x, y and z axis. <br>
 * <br>
 * A typical usage is to allow the class define the center and radius by calling either
 * <code>containAABB</code> or <code>averagePoints</code>. A call to <code>computeFramePoint</code>
...
public class BoundingBox extends BoundingVolume {
  ...
  @Override
  public Type getType() { return Type.AABB; }

  public void setXExtent(final double xExtent) { _xExtent = xExtent; }

  public double getXExtent() { return _xExtent; }

  public void setYExtent(final double yExtent) { _yExtent = yExtent; }

  public double getYExtent() { return _yExtent; }

  public void setZExtent(final double zExtent) { _zExtent = zExtent; }
  ...
  /**
   * <code>containAABB</code> creates a minimum-volume axis-aligned bounding box of the points, then
   * selects the smallest enclosing sphere of the box with the sphere centered at the boxes center.
   * 
   * @param points
   *          the list of points.
   */
  ...
  /**
   * <code>mergeLocal</code> combines this sphere with a second bounding sphere locally. Altering this
   * sphere to contain both the original and the additional sphere volumes;
   * 
   * @param volume
   *          the sphere to combine with this sphere.
   * @return this
  ...
  /**
   * <code>merge</code> combines this bounding box with another box which is defined by the center, x,
   * y, z extents.
   * 
   * @param boxCenter
  ...
  /**
   * <code>toString</code> returns the string representation of this object. The form is: "Radius:
   * RRR.SSSS Center: <Vector>".
   * 
  ...
  public boolean intersects(final ReadOnlyRay3 ray) {
    ...
    final boolean notEntirelyClipped =
        clip(direction.getX(), -diff.getX() - x, t) && clip(-direction.getX(), diff.getX() - x, t)
            && clip(direction.getY(), -diff.getY() - y, t) && clip(-direction.getY(), diff.getY() - y, t)
            && clip(direction.getZ(), -diff.getZ() - z, t) && clip(-direction.getZ(), diff.getZ() - z, t);
    ...
    if (notEntirelyClipped && (t[0] != 0.0 || t[1] != Double.POSITIVE_INFINITY)) {
      if (t[1] > t[0]) {
        ...
        final Vector3[] points =
            new Vector3[] {new Vector3(ray.getDirection()).multiplyLocal(distances[0]).addLocal(ray.getOrigin()),
  ...
  @Override
  public double getVolume() { return (8 * getXExtent() * getYExtent() * getZExtent()); }
}
