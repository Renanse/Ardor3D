...
/**
 * Curve class contains a list of control points and a spline. It also contains method for
 * visualising itself as a renderable series of points or a line.
 */
public class Curve {
  ...
  /**
   * Creates a new <code>Point</code> from the control points making up this curve. It will have the
   * name <code>point</code>, no normals, colour or texture, these can be added to the returned point
   * if needed.
   * 
   * @param steps
   *          The number of iterations to perform between control points, the higher this number the
   *          more points will be shown, but it will also contain more vertices, must be greater than
   *          one. Use two to just show the actual control points set for the curve.
  ...
  /**
   * Creates a new <code>Point</code> from the given control point indices. It will have the name
   * <code>point</code>, no normals, colour or texture, these can be added to the returned point if
   * needed.
   * 
   * @param start
   *          The index of the control point to start from, must be greater than or equal to one and
   *          less than <code>end</code>.
   * @param end
   *          The index of the control point to end with, must be less than
   *          {@link #getControlPointCount() controlPointCount} minus one and greater than
   *          <code>start</code>.
   * @param steps
   *          The number of iterations to perform between control points, the higher this number the
   *          more points will be shown, but it will also contain more vertices, must be greater than
  ...
  /**
   * Creates a new <code>Line</code> from the control points making up this curve. It will have the
   * name <code>curve</code>, no normals, colour or texture, these can be added to the returned line
   * if needed.
   * 
   * @param steps
   *          The number of iterations to perform between control points, the higher this number the
   *          smoother the returned line will be, but it will also contain more vertices, must be
   *          greater than one.
  ...
  /**
   * Creates a new <code>Line</code> from the given control point indices. It will have the name
   * <code>curve</code>, no normals, colour or texture, these can be added to the returned line if
   * needed.
   * 
   * @param start
   *          The index of the control point to start from, must be greater than or equal to one and
   ...
   * To calculate the length of a curve it must be interpolated (hence the steps parameter), this
   * method will do this EVERY time it's called (creating a lot of garbage vectors in the process).
   * This has been done for the sake of keeping this class simple and the code as readable as
   * possible. Therefore the length should be manually cached somewhere in your code if it is going to
   * be used repeatedly.
   * </p>
   * 
   * @param steps
   *          The number of iterations to perform between control points, the higher this number the
   *          more accurate the returned result will be.
   ...
   * @param t
   *          Should be between zero and one. Zero will return point <code>start</code> while one will
   *          return <code>end</code>, a value in between will return an interpolated vector between
   *          the two.
  ...
  public ReadOnlyVector3 interpolate(final int start, final int end, final double t, final Vector3 result) {
    ...
    return getSpline().interpolate(points.get(start - 1), points.get(start), points.get(end), points.get(end + 1), t,
        result);
  }
   ...
   */
  public int getControlPointCount() { return getControlPoints().size(); }
  ...
  public List<ReadOnlyVector3> getControlPoints() {
    ...
    assert (_controlPoints
        .size() >= 4) : "_controlPoints contained less than 4 elements, it must be contain at least 4 for this class to work!";
