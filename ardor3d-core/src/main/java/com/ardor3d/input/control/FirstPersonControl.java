...
public class FirstPersonControl {
  ...
  public ReadOnlyVector3 getUpAxis() { return _upAxis; }

  public void setUpAxis(final ReadOnlyVector3 upAxis) {
    _upAxis.set(upAxis);
  }

  public double getMouseRotateSpeed() { return _mouseRotateSpeed; }

  public void setMouseRotateSpeed(final double speed) { _mouseRotateSpeed = speed; }
  ...
  public void setKeyRotateSpeed(final double speed) { _keyRotateSpeed = speed; }
   ...
   * @param dragOnly
   *          if true, mouse input will only rotate the camera if one of the mouse buttons (left,
   *          center or right) is down.
  ...
  public void setupMouseTriggers(final LogicalLayer layer, final boolean dragOnly) {
    // Mouse look
    final Predicate<TwoInputStates> dragged = TriggerConditions.mouseMoved().and(TriggerConditions.leftButtonDown());
  ...
  public Predicate<TwoInputStates> setupKeyboardTriggers(final LogicalLayer layer) {
    // WASD control
    final Predicate<TwoInputStates> keysHeld = new Predicate<>() {
    ...
    final TriggerAction moveAction = (source, inputStates, tpf) -> FirstPersonControl.this
  ...
  public InputTrigger getKeyTrigger() { return _keyTrigger; }

  public InputTrigger getMouseTrigger() { return _mouseTrigger; }

  public boolean isClampVerticalAngle() { return _clampVerticalAngle; }

  /**
   * @param clampVerticalAngle
   *          if true, the vertical angle of the camera is locked between the minimum and maximum
   *          angles (default is [-60, 60])
   */
  public void setClampVerticalAngle(final boolean clampVerticalAngle) { _clampVerticalAngle = clampVerticalAngle; }

  public double getMinVerticalAngle() { return _minVerticalAngle; }

  /**
   * @param minVerticalAngle
   *          the new minimum angle, in radians, to clamp our vertical angle to. Defaults to -60
   *          degrees (in radians). Must be less than the max angle. Has no effect unless
   *          clampVerticalAngle is true.
   * @see #setClampVerticalAngle(boolean)
   */
  public void setMinVerticalAngle(final double minVerticalAngle) { _minVerticalAngle = minVerticalAngle; }

  public double getMaxVerticalAngle() { return _maxVerticalAngle; }

  /**
   *
   * @param maxVerticalAngle
   *          the new maximum angle, in radians, to clamp our vertical angle to. Defaults to +60
   *          degrees (in radians). Must be less than the max angle. Has no effect unless
   *          clampVerticalAngle is true.
   * @see #setClampVerticalAngle(boolean)
   */
  public void setMaxVerticalAngle(final double maxVerticalAngle) { _maxVerticalAngle = maxVerticalAngle; }
}
