...
/**
 * Implementation of a logical layer on top of the physical one, to be able to more easily trigger
 * certain commands for certain combination of user input.
 */
@ThreadSafe
public final class LogicalLayer {
  ...
  /**
   * Check all registered triggers to see if their respective conditions are met. For every trigger
   * whose condition is true, perform the associated action.
   * 
   * @param tpf
  ...
  public synchronized void checkTriggers(final double tpf) {
    for (final InputSource is : _inputs) {
      ...
      if (newStates.isEmpty()) {
        _applier.checkAndPerformTriggers(_triggers, is.source, new TwoInputStates(is.lastState, is.lastState), tpf);
      } else {
        ...
        for (final InputState inputState : newStates) {
          ...
          if (inputState != InputState.LOST_FOCUS) {
            _applier.checkAndPerformTriggers(_triggers, is.source, new TwoInputStates(is.lastState, inputState), time);
          }
  ...
  public void setApplier(final LogicalTriggersApplier applier) { _applier = applier; }

  public LogicalTriggersApplier getApplier() { return _applier; }
  ...
  public Set<InputTrigger> getTriggers() { return _triggers; }
