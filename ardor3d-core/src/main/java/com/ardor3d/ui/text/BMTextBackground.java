...
public class BMTextBackground extends Mesh implements BMTextChangeListener {
  ...
  /**
   * Sets the scale of the 9 slice border texture. By default this is set to 1 meaning we attempt to
   * make it pixel perfect. If the source texture is too large or small, leading to an undesired
   * border size, changing this value will adjust the border without scaling other transform values.
   */
  ...
  public float getBorderScale() { return _borderScale; }

  /**
   * Splits up the texture drawn on this background into a grid. x,y,z, and w of the given vector are
   * interpreted as the u/v coordinate offset from the left, top, right and bottom edges of the
   * texture respectively. This forms a 9 slice/patch grid. The edges of the grid are stretched along
   * their given axis to fit. The corners are presented without stretching their natural ratio. The
   * center patch is stretched to fit on both axis. The default value is (.25, .25, .25, .25) which
   * reserves a 25% border around the center content.
   */
  ...
  public ReadOnlyVector4 getTexBorderOffsets() { return _textureBorderOffsets; }
  ...
  public ReadOnlyVector4 getContentPadding() { return _contentPadding; }
  ...
  @Override
  public Class<? extends BMTextBackground> getClassTag() { return this.getClass(); }
