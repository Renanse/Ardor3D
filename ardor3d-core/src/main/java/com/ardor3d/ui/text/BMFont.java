...
public class BMFont implements Savable {
  ...
  /**
   * Reads an XML BMFont description file and loads corresponding texture. Note that the TGA written
   * by BMFont does not seem to be read properly by the Ardor3D loader. PNG works fine.
   *
   * @param source
   *          ResourceSource describing where this font comes from.
   * @param useMipMaps
   *          if true, use trilinear filtering with max anisotropy, else min filter is bilinear.
   *          MipMaps result in blurrier text, but less shimmering.
   * @throws IOException
  ...
  public void setBlendStateSetter(final RenderStateSetter setter) { _blendStateSetter = setter; }

  public void setAlphaStateSetter(final RenderStateSetter setter) { _alphaStateSetter = setter; }
  ...
  public int getSize() { return _info.size; }

  /**
   * The average of _common.base and _common.lineHeight seems to be the best general purpose value
   */
  public int getLineHeight() { return (_common.base + _common.lineHeight) / 2; }

  public int getTextureWidth() { return _common.scaleW; }

  public int getTextureHeight() { return _common.scaleH; }

  /**
   * @param chr
   *          integer character code
   * @return character descriptor for chr. If character is not in the char set, return '?' (if '?' is
   *         not in the char set, return will be null)
   */
   ...
   */
  public int getMaxCharAdvance() { return _maxCharAdv; }

  public int getOutlineWidth() { return _info.outline; }
  ...
  public Texture getPageTexture() { return _pageTexture; }

  public List<Integer> getMappedChars() { return new ArrayList<>(_charMap.keySet()); }

  public Map<Integer, Integer> getKerningsForCharacter(final int val) {
    return _kernMap.get(val);
  }

  public Map<Integer, Map<Integer, Integer>> getKerningMap() { return _kernMap; }
  ...
  public static class Info implements Savable {
    ...
    @Override
    public Class<?> getClassTag() { return getClass(); }
  }
