...
public class BMText extends Mesh {
  ...
  public enum AutoScale {
    ...
    /**
     * Do not auto scale if font screen size is smaller than native point size, otherwise maintain
     * native point size.
     */
    CapScreenSize;
  }
   ...
   * @param justify
   * @param useBlend
   *          if true: use alpha blending and use transparent render bucket, else if false: alpha test
   *          only and use opaque render bucket
   */
  public BMText(final String sName, final String text, final BMFont font, final Align align, final Justify justify,
  ...
  /**
   * If AutoScale is enabled, this scale parameter acts as a bias. Setting the scale to 0.95 will
   * sharpen the font and increase readability a bit if you're using a bilinear min filter on the
   * texture. When AutoScale is disabled, this scales the font to world units, e.g. setScale(1.0)
   * would make the font characters approximately 1 world unit in size, regardless of the font point
   * size.
   */
  ...
  public AutoScale getAutoScale() { return _autoScale; }

  public void setAutoFade(final AutoFade autoFade) { _autoFade = autoFade; }

  public AutoFade getAutoFade() { return _autoFade; }

  public void setAutoFadeFixedPixelSize(final int pixelSize) { _fixedPixelAlphaThresh = pixelSize; }

  public int getAutoFadeFixedPixelSize() { return _fixedPixelAlphaThresh; }

  /**
   * alpha falloff factor used when FixedPixelSize or CapScreenSize is used. Can be any positive
   * value; useful range is ~ 0-2
   * <ul>
   * <li>0 = transparent instantaneously
   * <li>1 = transparent when approximately 1/2 size
   * </ul>
   */
  public void setAutoFadeFalloff(final float factor) { _screenSizeAlphaFalloff = factor; }

  /**
   * @param nearOpaque
   *          text is completely opaque when distance between camera and text is less than this value
   * @param farTransparent
   *          text is completely transparent when distance between camera and text is greater than
   *          this value
   */
   ...
   */
  public void setAutoRotate(final boolean doAutoTransform) { _autoRotate = doAutoTransform; }

  public boolean getAutoRotate() { return _autoRotate; }
  ...
  /**
   * Set transparency based on distance from camera to text. if (distance < range.x) then opaque, if
   * (distance > range.y) then transparent, else lerp
   */
  ...
  /**
   * Check whether buffers have sufficient capacity to hold current string values; if not, increase
   * capacity and set the limit.
   *
   * @param text
   */
  ...
  public boolean getUseBlend() { return _useBlend; }
  ...
  public synchronized String getText() { return _textString; }
  ...
  public Align getAlign() { return _align; }
  ...
  public Justify getJustify() { return _justify; }
  ...
  public int getLineCount() { return _lines; }

  public ReadOnlyVector2 getFixedOffset() { return _fixedOffset; }

  public ReadOnlyVector2 getSize() { return _size; }
