...
/**
 * This class acts as a centralized data store for statistics. As data is added to the collector, a
 * sum total is kept as well as the total number of data samples given for the particular stat.
 */
public abstract class StatCollector {
  private static final Logger logger = Logger.getLogger(StatCollector.class.getName());

  /**
   * How many distinct past aggregate samples are kept before the oldest one is dropped on add. You
   * can multiply this by the time sample rate to determine the total history length in time.
   */
  protected static int maxSamples = 100;

  /**
   * Our map of current stat values. Current means values that have been collected within the current
  ...
  /**
   * Add a listener to the pool of listeners that are notified when a new stats aggregate is created
   * (at the end of each time sample).
   * 
   * @param listener
   *          the listener to add
   */
  ...
  /**
   * Removes a listener from the pool of listeners that are notified when a new stats aggregate is
   * created (at the end of each time sample).
   * 
   * @param listener
  ...
  /**
   * Removes a type from the set of stat types that are paid attention to when doing timed stat
   * checking.
   * 
   * @param type
   *          the listener to remove
  ...
  public static double getStartOffset() { return startOffset; }

  public static double getSampleRate() { return sampleRateMS; }

  public static void setSampleRate(final long sampleRateMS) { StatCollector.sampleRateMS = sampleRateMS; }

  public static void setMaxSamples(final int samples) { StatCollector.maxSamples = samples; }

  public static int getMaxSamples() { return maxSamples; }

  public static List<MultiStatSample> getHistorical() { return historical; }
  ...
  public static boolean isIgnoreStats() { return ignoreStats; }

  public static void setIgnoreStats(final boolean ignoreStats) { StatCollector.ignoreStats = ignoreStats; }
   ...
   * 
   * NOTE: You must ensure you are not inside a START/END timed block, (or you recreate any necessary
   * start calls) otherwise when endStat is called a stack exception will occur.
   */
