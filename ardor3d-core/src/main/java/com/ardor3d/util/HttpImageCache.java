...
public enum HttpImageCache {
    ...
    /**
     * @return the directory where we will store copies of downloaded images, in .ABI format. If null, file caching will
     *         be skipped. Defaults to <code>System.getProperty("user.dir") + "/cache/image/"</code>
     */
    ...
    /**
     * @param cacheDir
     *            the directory where we want to store copies of downloaded images, in .ABI format. If null, file
    ...
    /**
     * @return true if we should ask if a file we have cached locally has been recently modified. This action is taken
     *         if only if we find we already have the image and have not check in {@link #minModCheckSeconds} seconds.
     *         Defaults to false.
     */
     ...
     */
    public void setCheckModified(final boolean checkModified) {
        this.checkModified = checkModified;
    }

    /**
     * @return the maximum total size, in bytes, of the Images we will hold in our memory cache. When we hit this limit,
     *         we will flush items out of memory based on LRU logic. Default is 8 MB (8 * 1024 * 1024)
     */
     ...
     *            the maximum total size, in bytes, of the Images we will hold in our memory cache. When we hit this
     *            limit, we will flush items out of memory based on LRU logic.
     */
    ...
    /**
     * @return the minimum time, in seconds, between checking an image for remote modification. Default is 1 day (24 *
     *         60 * 60).
     */
    public long getMinModCheckSeconds() {
        return minModCheckSeconds;
    }
    ...
    /**
     * Attempt to retrieve an image, first from the local cache, then from the given URI. Runs asynchronously, calling
     * the supplied Consumer callback on completion.
     *
     * @param uri
     *            location of the image to download.
     * @param type
     *            the extension type of the image (e.g. ".png"). Used by ImageLoaderUtil when loading the image contents
     *            from the URI.
     * @param flipped
     *            if true, flip the image vertically before returning.
     * @param onSuccess
     *            Consumer callback called once we've downloaded the image. May be called with the argument
     *            <code>TextureState.getDefaultTextureImage()</code> if we are unable to load the image.
     */
     ...
     * @param flipped
     *            if true, flip the image vertically before returning.
     * @return The loaded Image, or <code>TextureState.getDefaultTextureImage()</code> if we are unable to load the
     *         image.
     */
