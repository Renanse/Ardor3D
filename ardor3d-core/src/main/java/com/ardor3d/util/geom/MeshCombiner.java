 ...
 * combine Mesh objects that have the same render states.
 *
 * XXX: should add in a way to combine only meshes with similar renderstates<br>
 * XXX: Might be able to reduce memory usage in the singular case where all sources do not have
 * indices defined (arrays).<br>
...
public class MeshCombiner {
  ...
  /**
   * <p>
   * Combine all mesh objects that fall under the scene graph the given source node. All Mesh objects
   * must have vertices and texcoords that have the same tuple width. It is possible to merge Mesh
   * objects together that have mismatched normals/colors/etc. (eg. one with colors and one without.)
   * </p>
   *
   * @param source
   *          our source node
  ...
  /**
   * Combine the given array of Mesh objects into a single Mesh. All Mesh objects must have vertices
   * and texcoords that have the same tuple width. It is possible to merge Mesh objects together that
   * have mismatched normals/colors/etc. (eg. one with colors and one without.)
   *
   * @param sources
  ...
  /**
   * Combine the given collection of Mesh objects into a single Mesh. All Mesh objects must have
   * vertices and texcoords that have the same tuple width. It is possible to merge Mesh objects
   * together that have mismatched normals/colors/etc. (eg. one with colors and one without.)
   *
   * @param sources
  ...
  public void addEntry(final MeshData source, final int vertexOffset) {
    ...
    if (source.getIndices() == null) {
      ...
      for (int i = 0, maxI = source.getSectionCount(); i < maxI; i++) {
        ...
        final int size = source.getIndexLengths() != null ? source.getIndexLengths()[i] : source.getVertexCount();
      ...
      for (int i = 0, maxI = source.getSectionCount(); i < maxI; i++) {
        // make an int array and populate it.
        final int size =
            source.getIndexLengths() != null ? source.getIndexLengths()[i] : source.getIndices().capacity();
        final int[] indices = new int[size];
        for (int j = 0; j < size; j++) {
