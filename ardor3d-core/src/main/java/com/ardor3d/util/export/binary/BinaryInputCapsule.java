...
public class BinaryInputCapsule implements InputCapsule {
   ...
   * UTF-8 crash course:
   *
   * UTF-8 codepoints map to UTF-16 codepoints and vv, which is what Java uses for it's Strings. (so a
   * UTF-8 codepoint can contain all possible values for a Java char)
   *
   * A UTF-8 codepoint can be 1, 2 or 3 bytes long. How long a codepoint is can be told by reading the
   * first byte: b < 0x80, 1 byte (b & 0xC0) == 0xC0, 2 bytes (b & 0xE0) == 0xE0, 3 bytes
   *
   * However there is an additional restriction to UTF-8, to enable you to find the start of a UTF-8
   * codepoint, if you start reading at a random point in a UTF-8 byte stream. That's why UTF-8
   * requires for the second and third byte of a multibyte codepoint: (b & 0x80) == 0x80 (in other
   * words, first bit must be 1)
   */
  ...
  protected String readString(final byte[] content) throws IOException {
    ...
    /*
     * We'll transfer the bytes into a separate byte array. While we do that we'll take the opportunity
     ...
     * as well as most 1 byte encodings relative to each other.
     *
     ...
     * use a "low" codepoint after a "high" codepoint - use a "low" codepoint after "high" codepoint,
     * after a "high" codepoint that starts with 1110
     *
     * In practice this means that unless 2 or 3 "high" codepoints are used after each other in proper
     * order, we'll detect the string was not originally UTF-8 encoded.
     */
    ...
    try {
      // even though so far the parsing might have been a legal UTF-8 sequence, only if a codepoint is
      // fully given
      // is it correct UTF-8
      ...
      // JavaDoc is vague about what happens when a decoding a String that contains un undecodable
      // sequence
      // it also doesn't specify which encodings have to be supported (though UTF-8 and ISO8859 have been
      // in the
