...
/**
 * <code>GameTaskQueue</code> is a simple queuing system to enqueue tasks that need to be
 * accomplished in a specific thread or phase of the application execution (for example, the OpenGL
 * rendering thread.) Upon sending in a task, the caller gets back a Future object useful for
 * retrieving a return from the Callable that was passed in.
 *
 * @see Future
 * @see Callable
 */
public class GameTaskQueue {
  ...
  private final List<ExecutionExceptionListener> _executionExceptionListeners =
      new LinkedList<>();
  ...
  /**
   * The state of this <code>GameTaskQueue</code> if it will execute all enqueued Callables on an
   * execute invokation.
   * 
   * @return boolean
   */
  public boolean isExecuteAll() { return _executeMultiple.get(); }

  /**
   * @param executeMultiple
   *          if false, only one task at most is executed per call to execute. If true, we will
   *          execute as many tasks as are available, bounded by the max execution time.
  ...
  /**
   * Sets the minimum amount of time the queue will execute tasks per frame. If this is set,
   * executeMultiple is automatically set to true and the execute() loop will execute as many tasks as
   * it can before the execution window threshold is passed. Any remaining tasks will be executed in
   * the following frame.
   * 
   * @param msecs
   *          the maximum number of milliseconds to start tasks. Note that this does not guarantee the
   *          tasks will finish under this time, only start.
   */
   ...
   * This is useful for checking the status of the task as well as being able to retrieve the return
   * object from Callable asynchronously.
   * 
   * @param <V>
   * @param callable
   * @return
  ...
  /**
   * Execute the tasks from this queue. Note that depending on the queue type, tasks may expect to be
   * run in a certain context (for example, the Render queue expects to be run from the Thread owning
   * a GL context.)
   */
  public void execute() {
    execute(null);
