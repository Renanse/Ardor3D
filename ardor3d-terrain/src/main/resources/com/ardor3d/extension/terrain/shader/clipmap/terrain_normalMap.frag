...
{  
	...
	vec4 norm = texture3DBilinear(normalMap, vec3(texCoord.x, texCoord.y, unit));
	vec4 norm2 = texture3DBilinear(normalMap, vec3(texCoord2.x, texCoord2.y, unit2));
...
//	float fog = clamp((gl_Fog.end - dist) * gl_Fog.scale, 0.0, 1.0);	
    ...
    vec4 color = vec4(0.1) + vec4(1.0) * NdotL;
