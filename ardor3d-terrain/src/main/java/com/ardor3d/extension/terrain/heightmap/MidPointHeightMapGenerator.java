...
public class MidPointHeightMapGenerator {
  ...
  /**
   * Constructor builds a new heightmap using the midpoint displacement algorithm. Roughness
   * determines how chaotic the terrain will be. Where 1 is perfectly self-similar, > 1 early
   * iterations have a disproportionately large effect creating smooth terrain, and < 1 late iteraions
   * have a disproportionately large effect creating chaotic terrain.
   * 
   * @param size
  ...
  /**
   * <code>load</code> generates the heightfield using the Midpoint Displacement algorithm.
   * <code>load</code> uses the latest attributes, so a call to <code>load</code> is recommended if
   * attributes have changed using the set methods.
   */
    ...
    while (counter > 0) {
      ...
      for (int i = 0; i < size; i += counter) {
        for (int j = 0; j < size; j += counter) {
          ...
          tempBuffer[mi][mj] = (tempBuffer[i][j] + tempBuffer[ni][j] + tempBuffer[i][nj] + tempBuffer[ni][nj]) / 4
      ...
      for (int i = 0; i < size; i += counter) {
        for (int j = 0; j < size; j += counter) {
          ...
          tempBuffer[mi][j] = (tempBuffer[i][j] + tempBuffer[ni][j] + tempBuffer[mi][pmj] + tempBuffer[mi][mj]) / 4
              + random.nextFloat() * height - height / 2;

          // Calculate the square value for the left side of the rectangle
          tempBuffer[i][mj] = (tempBuffer[i][j] + tempBuffer[i][nj] + tempBuffer[pmi][mj] + tempBuffer[mi][mj]) / 4
              + random.nextFloat() * height - height / 2;
  ...
  /**
   * <code>setRoughness</code> sets the new roughness value of the heightmap. Roughness determines how
   * chaotic the terrain will be. Where 1 is perfectly self-similar, > 1 early iterations have a
   * disproportionately large effect creating smooth terrain, and < 1 late iteraions have a
   * disproportionately large effect creating chaotic terrain.
   * 
   * @param roughness
  ...
  /**
   * <code>normalizeTerrain</code> takes the current terrain data and converts it to values between 0
   * and NORMALIZE_RANGE.
   * 
   * @param tempBuffer
  ...
  /**
   * <code>erodeTerrain</code> is a convenience method that applies the FIR filter to a given height
   * map. This simulates water errosion.
   * 
   * @param tempBuffer
  ...
  public float getHeightRange() { return heightRange; }

  public void setHeightRange(final float heightRange) { this.heightRange = heightRange; }
}
