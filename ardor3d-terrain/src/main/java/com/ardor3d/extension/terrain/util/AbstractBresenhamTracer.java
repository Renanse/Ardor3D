...
/**
 * AbstractBresenhamTracer is a simple base class for using Bresenham's line equation. Bresenham's
 * line equation is useful for doing various tasks that involve regularly spaced grids (such as
 * picking against a height-map.) This class is not intended to do any picking, only traveling and
 * reporting the grid squares traversed.
 */
public abstract class AbstractBresenhamTracer {
   ...
   */
  public Direction getLastStepDirection() { return _stepDirection; }

  /**
   * @return the row and column we are currently in on the grid.
   */
  public int[] getGridLocation() { return _gridLocation; }

  /**
   * @return the total length we have traveled from the origin of our ray set in startWalk.
   */
  public double getTotalTraveled() { return _totalTravel; }

  /**
   * Set the world origin of our grid. This is useful to the tracer when doing conversion between
   * world coordinates and grid locations.
   * 
   * @param origin
   ...
   */
  public Vector3 getGridOrigin() { return _gridOrigin; }

  /**
   * Set the world spacing (scale) of our grid. Also useful for converting between world coordinates
   * and grid location.
   * 
   * @param spacing
   ...
   */
  public Vector3 getGridSpacing() { return _gridSpacing; }
  ...
  /**
   * @return true if our walkRay, specified in startWalk, ended up being perpendicular to the grid
   *         (and therefore can not move to a new grid location on calls to next(). You should test
   *         this after calling startWalk and before calling next().
   */
