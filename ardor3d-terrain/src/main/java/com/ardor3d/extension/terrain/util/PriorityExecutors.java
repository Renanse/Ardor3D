...
public class PriorityExecutors {
    ...
    private static class PriorityExecutor extends ThreadPoolExecutor {
        ...
        public PriorityExecutor(final int corePoolSize, final int maximumPoolSize, final long keepAliveTime,
            ...
            super(corePoolSize, maximumPoolSize, keepAliveTime, unit,
                    (BlockingQueue) new PriorityBlockingQueue<ComparableTask>(100,
                            ComparableTask.comparatorByPriorityAndSequentialOrder()), factory);
        }
        ...
        private <T> PriorityCallable<T> ensurePriorityCallable(final Callable<T> callable) {
            return callable instanceof PriorityCallable ? (PriorityCallable<T>) callable : PriorityCallable.of(
                    callable, DEFAULT_PRIORITY);
        }

        private PriorityRunnable ensurePriorityRunnable(final Runnable runnable) {
            return runnable instanceof PriorityRunnable ? (PriorityRunnable) runnable : PriorityRunnable.of(runnable,
                    DEFAULT_PRIORITY);
        }
    ...
    public interface PriorityCallable<V> extends Callable<V>, HasPriority {
        ...
        public static <V> PriorityCallable<V> of(final Callable<V> callable, final int priority) {
    ...
    public interface PriorityRunnable extends Runnable, HasPriority {
        ...
        public static PriorityRunnable of(final Runnable runnable, final int priority) {
