  ...
  public static void applyFunction(final boolean targetAlpha, final SourceCacheFunction function,
    ...
    if (function == null) {
      switch (format) {
        ...
        case RGB8:
          if (!targetAlpha) {
            RGB8ToRGBFunction.getInstance().doConversion(sourceData, store, destX, destY, dataSize, tileSize);
            return;
          } else {
            RGB8ToRGBAFunction.getInstance().doConversion(sourceData, store, destX, destY, dataSize, tileSize);
            return;
          }
        case RGBA8:
          if (!targetAlpha) {
            RGBA8ToRGBFunction.getInstance().doConversion(sourceData, store, destX, destY, dataSize, tileSize);
            return;
          } else {
            RGBA8ToRGBAFunction.getInstance().doConversion(sourceData, store, destX, destY, dataSize, tileSize);
            return;
          }
        default:
