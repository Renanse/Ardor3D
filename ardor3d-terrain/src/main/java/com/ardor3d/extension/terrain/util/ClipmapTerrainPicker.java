...
public class ClipmapTerrainPicker {
  ...
  public ClipmapTerrainPicker(final List<ClipmapLevel> levels,
    final Class<? extends AbstractBresenhamTracer> tracerClass, final int maxChecks, final Vector3 initialSpacing)
      throws Ardor3dException {
  ...
  public Vector3 getTerrainIntersection(final ReadOnlyTransform terrainWorldTransform, final ReadOnlyVector3 eyePos,
    ...
    if (tracer.isRayPerpendicularToGrid()) {
      ...
      level.getCache().getEyeCoords(tileStore, tracer.getGridLocation()[0], tracer.getGridLocation()[1], _workEyePos);
      ...
      final float h4 =
          getWeightedHeight(tileStore[12], tileStore[13], tileStore[14], tileStore[15], scaledClipSideSize);

      final double x = _workEyePos.getX();
      final double z = _workEyePos.getZ();
      final double intOnX = x - Math.floor(x), intOnZ = z - Math.floor(z);
      final double height = MathUtils.lerp(intOnZ, MathUtils.lerp(intOnX, h1, h2), MathUtils.lerp(intOnX, h3, h4));
    ...
    while (iter < _maxChecks) {
      ...
      if (checkTriangles(tracer.getGridLocation()[0], tracer.getGridLocation()[1], intersection, normalStore, tracer,
          level, _workEyePos)) {
      ...
      if (checkTriangles(tracer.getGridLocation()[0] + dx, tracer.getGridLocation()[1] + dy, intersection, normalStore,
  ...
  public int getMaxChecks() { return _maxChecks; }

  public void setMaxChecks(final int max) { _maxChecks = max; }

  public List<ClipmapLevel> getPyramid() { return _clipmapLevels; }
  ...
  private boolean checkTriangles(final int gridX, final int gridY, final Vector3 store, final Vector3 normalStore,
    ...
    if (!_workRay.intersectsTriangle(_gridTriA.getA(), _gridTriA.getB(), _gridTriA.getC(), store)) {
      final boolean intersects =
          _workRay.intersectsTriangle(_gridTriB.getA(), _gridTriB.getB(), _gridTriB.getC(), store);
  ...
  /**
   * Calculate the triangles (in world coordinate space) of a Pyramid that correspond to the given
  ...
  private boolean getTriangles(final int gridX, final int gridY, final AbstractBresenhamTracer tracer,
    ...
    final float h2 = getWeightedHeight(tileStore[4], tileStore[5], tileStore[6], tileStore[7], scaledClipSideSize);
    final float h3 = getWeightedHeight(tileStore[8], tileStore[9], tileStore[10], tileStore[11], scaledClipSideSize);
    final float h4 = getWeightedHeight(tileStore[12], tileStore[13], tileStore[14], tileStore[15], scaledClipSideSize);
  ...
  public void setMaxLevel(final int maxLevel) { _maxLevel = maxLevel; }

  public int getMaxLevel() { return _maxLevel; }

  public void setMinLevel(final int minLevel) { _minLevel = minLevel; }
