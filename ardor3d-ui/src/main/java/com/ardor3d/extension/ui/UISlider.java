...
public class UISlider extends UIContainer {
   ...
   * @param maxValue
   *          the maximum value the slider can take (inclusive). Must be greater than or equal to
   *          minValue.
   * @param initialValue
  ...
  public void layout() {
    ...
    // Set the backing panel's position and size based on orientation. We'll center it on the
    // perpendicular axis.
  ...
  private void updateKnob() {
    if ((float) (_model.getMaxValue() - _model.getMinValue()) != 0) {
      _knob.setPosition(
          (_model.getCurrentValue() - _model.getMinValue()) / (float) (_model.getMaxValue() - _model.getMinValue()));
    } else {
      _knob.setPosition(0);
    }
  }
   ...
   */
  public int getValue() { return _model.getCurrentValue(); }

  /**
   * @return the data model for the slider.
   */
  public SliderModel getModel() { return _model; }
  ...
  void knobReleased() {
    if (_snapToValues) {
      setValue(Math.round(_knob.getPosition() * (_model.getMaxValue() - _model.getMinValue())) + _model.getMinValue());
    }
  }
