...
/**
 * This layout places components on the edges or in the center of a container, depending on the
 * value of the layout data object they hold. The behavior is meant to be similar to awt's
 * {@link java.awt.BorderLayout BorderLayout}.
 *
      ...
      if (data != null) {
        switch (data) {
          ...
          case EAST:
            ...
            comp.getRelativeComponentBounds(store);
            comp.setLocalXY(container.getContentWidth() - store.getWidth() - 1 - store.getX(),
                heightSouth - store.getY());
            break;
          case WEST:
            comp.fitComponentIn(store.getWidth(), container.getContentHeight() - heightNorth - heightSouth);
            comp.getRelativeComponentBounds(store);
