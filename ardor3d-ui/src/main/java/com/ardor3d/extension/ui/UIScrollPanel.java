...
public class UIScrollPanel extends UIPanel {
  ...
  public UIScrollPanel(final UIComponent view) {
    ...
    setDoClip(true);
    horizontalScrollBar.addActionListener(event -> {
      offsetX = horizontalScrollBar.getValue();
      fireComponentDirty();
      updateScrollBarSliders();
    });
    verticalScrollBar.addActionListener(event -> {
  ...
  private void updateScrollBarSliders() {
    if (view != null) {
      ...
      verticalScrollBar.setMaxValue(
          view.getLocalComponentHeight() - getContentHeight() + horizontalScrollBar.getLocalComponentHeight());
      horizontalScrollBar.setValue(offsetX);
      horizontalScrollBar
          .setMaxValue(view.getLocalComponentWidth() - getContentWidth() + verticalScrollBar.getLocalComponentWidth());
