...
/**
 * Defines a component that can hold and manage other components or containers, using a layout
 * manager to position and potentially resize them.
 */
public abstract class UIContainer extends UIComponent {
  ...
  /**
   * A flag indicating that some part of this container needs repainting. On the next draw call, we
   * should update our cached texture, if using one.
   */
  private boolean _dirty = true;
  /**
   * If true, use a cached texture to display this container (on a simple quad) instead of drawing all
   * of its components.
   */
  ...
  /**
   * Removes all UI components from this container. If other types of Spatials are attached to this
   ...
   */
  public void setLayout(final UILayout layout) { _layout = layout; }

  /**
   * @return the layout currently used by this container or null if no layout is used.
   */
  public UILayout getLayout() { return _layout; }
  ...
  public synchronized void draw(final Renderer renderer) {
    ...
    // Calculate our standin's translation (and size) so that we are drawn in the bottom left corner of
    // the texture.
  ...
  private void buildStandin(final Renderer renderer) {
    ...
    if (UIContainer._textureRenderer == null) {
      ...
      final int size =
          maxRBS > 0 ? Math.min(UIContainer.STANDIN_TEXTURE_SIZE, maxRBS) : UIContainer.STANDIN_TEXTURE_SIZE;
  ...
  /**
   * Causes our shared texture renderer - used to draw cached versions of all containers - to be
   * recreated on the next render loop.
   */
   ...
   */
  public void setDirty(final boolean dirty) { _dirty = dirty; }
  ...
  /**
   * Release our standin and cached texture for gc. If needed again, they will be created from
   * scratch.
   */
   ...
   *
   * @param doClip
   *          true (default) if we want this container to clip the drawing of its contents to the
   *          dimensions of its content area.
   */
  public void setDoClip(final boolean doClip) { _doClip = doClip; }

  public boolean isDoClip() { return _doClip; }

  /**
   * @param use
   *          if true, we will draw the container's contents to a cached texture and use that to
   *          display this container (on a simple quad) instead of drawing all of its components each
   *          time. When the container is marked as dirty, we will update the contents of the texture.
   */
   ...
   */
  public boolean isUseStandin() { return _useStandin; }

  /**
   * @return true if we are currently rendering a container to texture.
   */
  public static boolean isDrawingStandin() { return UIContainer._drawingStandin; }
