...
/**
 * This border takes a set of images and draws them around the edge of a UI component. There are
 * eight possible border images - 4 for the sides of the component and 4 for the corners. Of these,
 * only the sides must be provided. If the corners are null, the top and bottom will stretch to fill
 * in the gaps.
 */
public class ImageBorder extends UIBorder {
  ...
  public ImageBorder(final SubTex subtex) {
    ...
    if (top > 0) {
      _topEdge =
          new SubTex(subtex.getTexture(), subtex.getX() + left, subtex.getY(), subtex.getWidth() - left - right, top);
      if (left > 0) {
        _topLeftCorner = new SubTex(subtex.getTexture(), subtex.getX(), subtex.getY(), left, top);
      }
      if (right > 0) {
        _topRightCorner =
            new SubTex(subtex.getTexture(), subtex.getX() + subtex.getWidth() - right, subtex.getY(), right, top);
      }
    } else {
    ...
    if (left > 0) {
      _leftEdge =
          new SubTex(subtex.getTexture(), subtex.getX(), subtex.getY() + top, left, subtex.getHeight() - top - bottom);
    } else {
      _leftEdge = new SubTex(subtex.getTexture(), 0, 0, 0, 0);
    }

    if (right > 0) {
      _rightEdge = new SubTex(subtex.getTexture(), subtex.getX() + subtex.getWidth() - right, subtex.getY() + top,
          right, subtex.getHeight() - top - bottom);
    } else {
    ...
    if (bottom > 0) {
      ...
      _bottomEdge =
          new SubTex(subtex.getTexture(), subtex.getX() + left, botY, subtex.getWidth() - left - right, bottom);
      if (left > 0) {
        _bottomLeftCorner = new SubTex(subtex.getTexture(), subtex.getX(), botY, left, bottom);
      }
      if (right > 0) {
        _bottomRightCorner =
            new SubTex(subtex.getTexture(), subtex.getX() + subtex.getWidth() - right, botY, right, bottom);
      }
    } else {
  ...
  public SubTex getBottomEdge() { return _bottomEdge; }

  public void setBottomEdge(final SubTex bottomEdge) { _bottomEdge = bottomEdge; }

  public SubTex getBottomLeftCorner() { return _bottomLeftCorner; }

  public void setBottomLeftCorner(final SubTex bottomLeftCorner) { _bottomLeftCorner = bottomLeftCorner; }

  public SubTex getBottomRightCorner() { return _bottomRightCorner; }

  public void setBottomRightCorner(final SubTex bottomRightCorner) { _bottomRightCorner = bottomRightCorner; }

  public SubTex getLeftEdge() { return _leftEdge; }

  public void setLeftEdge(final SubTex leftEdge) { _leftEdge = leftEdge; }

  public SubTex getRightEdge() { return _rightEdge; }

  public void setRightEdge(final SubTex rightEdge) { _rightEdge = rightEdge; }

  public SubTex getTopEdge() { return _topEdge; }

  public void setTopEdge(final SubTex topEdge) { _topEdge = topEdge; }

  public SubTex getTopLeftCorner() { return _topLeftCorner; }

  public void setTopLeftCorner(final SubTex topLeftCorner) { _topLeftCorner = topLeftCorner; }

  public SubTex getTopRightCorner() { return _topRightCorner; }

  public void setTopRightCorner(final SubTex topRightCorner) { _topRightCorner = topRightCorner; }
  ...
  public void draw(final Renderer renderer, final UIComponent comp) {
    ...
    if (_topRightCorner != null) {
      SubTexUtil.drawSubTex(renderer, _topRightCorner, dX + (borderWidth - _topRightCorner.getWidth()),
          dY + (borderHeight - _topRightCorner.getHeight()), _topRightCorner.getWidth(), _topRightCorner.getHeight(),
          comp.getWorldTransform());
    }
