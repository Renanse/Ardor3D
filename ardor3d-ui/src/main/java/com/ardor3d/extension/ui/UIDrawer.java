...
public class UIDrawer extends UIContainer {
    ...
    /**
     * The top, hidden panel of the drawer, holding the title bar, allowing it to be smaller than the drawer contents.
     */
    protected final UIPanel _titleBarContainer;

    /** The drag listener responsible for allowing repositioning of the drawer by dragging the title label. */
    ...
    public UIDrawer(final String title, final DrawerEdge edge, final EnumSet<DrawerButtons> buttons) {
        ...
        _titleBarContainer
                .setLayout(new RowLayout(_edge == DrawerEdge.TOP || _edge == DrawerEdge.BOTTOM, false, false));
    ...
    public boolean isDraggable() {
        return _draggable && !_dragInterupt;
    }

    public static double getSlideSpeed() {
        return UIDrawer._slideSpeed;
    }

    public static void setSlideSpeed(final double slideSpeed) {
        UIDrawer._slideSpeed = slideSpeed;
    }
    ...
    protected void applyExpansionAnimation() {
        ...
        switch (getEdge()) {
            ...
            case TOP:
                start = getHudY();
                target = getHud().getHeight()
                        + (_expanded ? -getLocalComponentHeight() : -_titleBar.getLocalComponentHeight());
                break;
            case LEFT:
                start = getHudX();
            ...
            case RIGHT:
                start = getHudX();
                target = getHud().getWidth()
                        + (_expanded ? -getLocalComponentWidth() : -_titleBar.getLocalComponentWidth());
                break;
            default:
                return;
        }
