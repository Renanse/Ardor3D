...
public class UISliderKnob extends UIContainer {
   ...
   */
  public UILabel getKnobLabel() { return _knobLabel; }
  ...
  /**
   * @return the relative location (x or y) of the leading edge of the knob handle. An x value if the
   *         we are horizontal or a y value if we are vertical. This is used for various mouse
   *         interactions.
   */
  ...
  private class KnobDragListener implements DragListener {
    ...
    public void drag(final int x, final int y) {
      ...
      model.setCurrentValue(
          Math.round(getPosition() * (model.getMaxValue() - model.getMinValue())) + model.getMinValue(), _parentSlider);
    }
