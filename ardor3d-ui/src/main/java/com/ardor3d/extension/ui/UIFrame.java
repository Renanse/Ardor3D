...
/**
 * A component similar to an inner frame in Swing. It can be dragged around the screen, minimized,
 * expanded, closed, and resized. Frames can also have their opacity individually assigned which
 * will affect all elements drawn within them.
 */
public class UIFrame extends UIContainer {
  ...
  /**
   * The drag listener responsible for allowing repositioning of the frame by dragging the title
   * label.
   */
  ...
  /**
   * @param decorated
   *          true to show the title and status bars. False to remove both. Undecorated frames have no
   *          resize or drag handles, or close buttons, etc.
   */
   ...
   */
  public boolean isDecorated() { return _decorated; }

  /**
   * @param resizeable
   *          true if we should allow resizing of this frame via a resize handle in the status bar.
   *          This does not stop programmatic resizing of this frame.
   */
   ...
   */
  public boolean isResizeable() { return _resizeable; }
   ...
   */
  public UIFrameBar getTitleBar() { return _titleBar; }

  /**
   * @return this frame's status bar
   */
  public UIFrameStatusBar getStatusBar() { return _statusBar; }
   ...
   */
  public UIPanel getBasePanel() { return _basePanel; }
  ...
  public void pack() {
    ...
    final int width =
        _contentPanel.getMinimumLocalComponentWidth() + _basePanel.getTotalLeft() + _basePanel.getTotalRight();
    int height =
        _contentPanel.getMinimumLocalComponentHeight() + _basePanel.getTotalTop() + _basePanel.getTotalBottom();
