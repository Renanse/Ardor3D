...
public abstract class TextSelection {
  ...
  public SelectionState getState() { return _state; }

  public void setState(final SelectionState state) { _state = state; }
  ...
  private void updateMesh() {
    ...
    for (int j = 0; !exit && j < data._lineEnds.size(); j++) {
      ...
      if (_endIndex <= end) {
        ...
        xEnd = data._xStarts.get(_endIndex - 1) + (int) Math.round(charDesc.getScale() * charDesc.getXAdvance());
        exit = true;
      } else {
