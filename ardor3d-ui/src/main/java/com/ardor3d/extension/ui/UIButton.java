...
public class UIButton extends AbstractLabelUIComponent {
   ...
   */
  public boolean isPressed() { return getCurrentState().equals(getPressedState()); }

  /**
   * @return true if the mouse over state is our current state.
   */
  public boolean isMouseOver() { return getCurrentState().equals(getMouseOverState()); }
  ...
  public LabelState getMouseOverState() { return _mouseOverState; }

  public LabelState getMouseOverSelectedState() { return _mouseOverSelectedState; }

  public LabelState getPressedState() { return _pressedState; }

  public LabelState getSelectedState() { return _selectedState; }

  public LabelState getDisabledSelectedState() { return _disabledSelectedState; }
  ...
  /**
   * If selectable, set this button's state to either selected (true) or default (false) - or disabled
   * versions of each if currently disabled.
   *
   * @param selected
   */
  ...
  /**
   * @param group
   *          the group this button should belong to. Removes the button from any group it currently
   *          belongs to.
   */
   ...
   */
  public ButtonGroup getGroup() { return _group; }

  /**
   * @param enabled
   *          if true, sets the defaultState to active, otherwise sets the disabledState or
   *          disabledSelectedState to active.
   */
  @Override
  ...
  public void setActionCommand(final String actionCommand) { _actionCommand = actionCommand; }
  ...
  class DefaultState extends LabelState {
    ...
    public void mouseEntered(final int mouseX, final int mouseY, final InputState state) {
      final UIHud hud = UIButton.this.getHud();
      final boolean drag = hud != null && hud.isDragging(state);
      if (!drag) {
        switchState(getMouseOverState());
        return;
      } else if (hud.getFocusedComponent() == UIButton.this) {
  ...
  class SelectedState extends LabelState {
    ...
    public void mouseEntered(final int mouseX, final int mouseY, final InputState state) {
      final UIHud hud = UIButton.this.getHud();
      final boolean drag = hud != null && hud.isDragging(state);
      if (!drag || hud.getFocusedComponent() == UIButton.this) {
        switchState(getMouseOverSelectedState());
        return;
      }
    }
  ...
  public Set<UIState> getStates() {
    return Set.of(_defaultState, _disabledState, _pressedState, _selectedState,
