...
/**
 * WaveFront OBJ exporter. It supports only the meshes. Several meshes can be exported into the same
 * OBJ file. Only a few kinds of primitives are supported. N.B: If the texture is flipped in
 * Ardor3D, you will have to flip it manually when loading the resulting OBJ file.
 *
...
public class ObjExporter {
        ...
        if (mesh != null) {
          if (mesh.getControllerCount() == 0 || !(mesh.getController(0) instanceof KeyframeController<?> controller)) {
            ...
            for (final Mesh submesh : subMeshList) {
              save(submesh, objFile, mtlFile, !firstFiles, firstVertexIndex, firstFiles, materialList, textureName);
              firstFiles = false;
   ...
   * @param firstFiles
   *          indicates whether the couple of files is used for the first time, i.e there is nothing
   *          to append despite the value of <code>append</code>
   * @param materialList
    ...
    try {
      ...
      if (mtlFile != null) {
        ...
        if (sameObjMtl == null) {
          ...
          mtlName =
              mtlFile.getName().trim().replaceAll(" ", "") + "_" + (materialList == null ? 1 : materialList.size() + 1);
      ...
      if (textureState.isEnabled() && textureState.getTexture() != null) {
        ...
        if (tKey.isFlipped()) {
          ObjExporter.logger.warning(
              "The texture " + tmpTextureName + " will have to be flipped manually when loading this OBJ file");
        } else {
          ObjExporter.logger.warning(
              "The texture " + tmpTextureName + " might need to be flipped manually when loading this OBJ file");
        }
      } else {
        textureName = null;
      }
    ...
    if (data != null) {
      if (keyword == null || keyword.isEmpty()) {
        ...
        if (tupleCount < expectedTupleCount) {
          throw new IllegalArgumentException("[" + keyword + "] not enough data to match with the vertex count: "
              + tupleCount + " < " + expectedTupleCount);
        } else {
          if (tupleCount > expectedTupleCount) {
            ObjExporter.logger.warning("[" + keyword + "] too much data to match with the vertex count: " + tupleCount
                + " > " + expectedTupleCount + ". Skips useless tuple(s)");
          }
        }
