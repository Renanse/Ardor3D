...
public class Md2Importer {
  ...
  public Md2DataStore load(final ResourceSource resource) {
    ...
    try {
      ...
      // parse the header
      final Md2Header header = new Md2Header(bis.readInt(), bis.readInt(), bis.readInt(), bis.readInt(), bis.readInt(),
          bis.readInt(), bis.readInt(), bis.readInt(), bis.readInt(), bis.readInt(), bis.readInt(), bis.readInt(),
      ...
      for (int i = 0; i < header.numGlCommands; i++) {
        ...
        commands[i] = cmd = new Md2GlCommand(length >= 0 ? IndexMode.TriangleStrip : IndexMode.TriangleFan, absLength);
