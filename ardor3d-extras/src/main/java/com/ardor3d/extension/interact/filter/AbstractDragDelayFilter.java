...
/**
 * UpdateFilter useful for delaying drag interactions. Delaying the drag interaction is useful in situations such as
...
public abstract class AbstractDragDelayFilter extends UpdateFilterAdapter {
    ...
    @Override
    public void beginDrag(final InteractManager manager, final AbstractInteractWidget widget,
     ...
     *            the current MouseState involved in the interaction
     * @param percent
     *            the progress, as a percent, made so far in the delay prior to allowing the drag. In the range, [0f,
     *            1f]
     */
    ...
    /**
     * Clear our timer visualization, possibly adding an exit animation, if we don't need to immediately clear.
     *
     * @param manager
     *            the InteractManger being used.
     * @param widget
     ...
     *            the current MouseState involved in the interaction
     * @param immediate
     *            if true, immediately clear the time visualization. If false, it is okay to play some exit
     *            animation/etc. before clearing the timer.
     */
