...
/**
 * A tool that uses the AtlasNode/AtlasPacker algorithm to pack textures into texture atlases. It
 * modifies the uv coordinates of the meshes, and tries to pack the atlases in a way that works with
 * the wrap modes of the textures involved.
 * <p>
 * Simple use case:
 * <p>
 * <blockquote>
 ...
 * packer.insert(mesh2);
 *
 * // Create all the atlases (also possible to set filters etc here through the
 * // AtlasTextureParameter)
...
public class TexturePacker {
  ...
  public void insert(final TextureParameter parameterObject) {
    ...
    if (parameterObject.getTexture() == null) {
      TexturePacker.logger.warning("Skipping mesh! - No texture found at index " + parameterObject.getTextureIndex()
          + " for mesh: " + parameterObject);
      return;
    }
    ...
    if (format != ImageDataFormat.RGB && format != ImageDataFormat.RGBA) {
      TexturePacker.logger
          .warning("Skipping mesh! - Only RGB and RGBA texture formats supported currently: " + parameterObject);
      return;
    }
  ...
  public void createAtlases(final AtlasTextureParameter atlasTextureParameter) {
    for (final ByteBuffer data : dataBuffers) {
      ...
      final Texture texture = TextureManager.loadFromImage(image, atlasTextureParameter.minificationFilter, format);
