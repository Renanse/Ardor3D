...
public class ColladaNodeUtils {
  ...
  public Node getVisualScene(final Element colladaRoot) {
    ...
    if (visualScene != null) {
      final Node sceneRoot = new Node(
          visualScene.getAttributeValue("name") != null ? visualScene.getAttributeValue("name") : "Collada Root");
  ...
  /**
   * @param instanceNode
   * @return a new Ardor3D node, created from the <node> pointed to by the given <instance_node>
   *         element
   */
  ...
  protected void createJointAttachment(final JointNode jointChildNode, final Node node, final Node subNode) {
    final AttachmentPoint attach =
        new AttachmentPoint("attach-" + node.getName(), (short) 0, subNode, new Transform(subNode.getTransform()));
