 ...
 * arrays extracted during sax parsing.
 */
public class ColladaDOMUtil {
   ...
   * @param query
   *          XPath expression
   * @return the list of selected items, which may be of types: {@link Element}, {@link Attribute},
  ...
  public ColorRGBA getColor(final String colorDescription) {
    ...
    if (values.length < 3 || values.length > 4) {
      throw new ColladaException(
          "Expected color definition of length 3 or 4 - got " + values.length + " for description: " + colorDescription,
          colorDescription);
    }
  ...
  public Element getPositionSource(final Element v) {
    ...
    // changed this to throw an exception instead - otherwise, there will just be a nullpointer
    // exception
