...
public class ColladaMeshUtils {
  ...
  /**
   * Builds a mesh from a Collada geometry element. Currently supported mesh types: mesh, polygons,
   * polylist, triangles, lines. Not supported yet: linestrips, trifans, tristrips. If no meshtype is
   * found, a pointcloud is built.
   *
  ...
  public Mesh buildMeshPolygons(final Element colladaGeometry, final Element polys) {
    ...
    final IndexBufferData<?> meshIndices =
        BufferUtils.createIndexBufferData(numIndices, polyMesh.getMeshData().getVertexCount() - 1);
   ...
   * @param pipes
   * @param currentVal
   * @return the vertex index referenced in the given indices based on the pipes. Integer.MIN_VALUE is
   *         returned if no vertex pipe is found.
   */
  ...
  /**
   * Extract name from xml element, some exporters don't support 'name' attribute, so we better use
   * the material instead of a generic name.
   *
   * @param element
