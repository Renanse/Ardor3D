...
public class ColladaMaterialUtils {
  ...
  public void applyMaterial(final String materialName, final Mesh mesh) {
    ...
    if ("effect".equals(effectNode.getName())) {
      /*
       * temp cache for textures, we do not want to add textures twice (for example, transparant map might
       * point to diffuse texture)
       */
      ...
      if (common != null) {
            ...
            if (technique.getChild(type) == null) {
              ...
              if (technique.getChild(type) == null) {
                ColladaMaterialUtils.logger
                    .warning("COMMON material has unusuable techinque. " + child.getAttributeValue("url"));
                return;
              }
            }
          }
        ...
        if (property != null) {
          ...
          if ("color".equals(propertyValue.getName())) {
            ...
            diffuseTexture = populateTextureState(mesh, propertyValue, effect, loadedTextures, mInfo, "diffuse");
          }
        }
        /* Ambient property */
        ...
        if (property != null) {
          ...
          if ("float".equals(propertyValue.getName())) {
            ...
            if (shininess >= 0.0f && shininess <= 1.0f) {
              ...
              shininess *= 128;
              logger.finest(
                  "Shininess - " + oldShininess + " - was in the [0,1] range. Scaling to [0, 128] - " + shininess);
            } else if (shininess < 0 || shininess > 128) {
              final float oldShininess = shininess;
              shininess = MathUtils.clamp(shininess, 0, 128);
              logger.warning(
                  "Shininess must be between 0 and 128. Shininess " + oldShininess + " was clamped to " + shininess);
            }
      ...
      ColladaMaterialUtils.logger
          .warning("material effect not found: " + mat.getChild("instance_material").getAttributeValue("url"));
    }
  }

  /**
   * Function to searches an xml node for <texture> elements and adds them to the texture state of the
   * mesh.
   *
   * @param mesh
  ...
  private Texture loadTexture2D(final String path, final Texture.MinificationFilter minFilter) {
    ...
    if (_importer.getTextureLocator() == null) {
      texture = TextureManager.load(path, minFilter,
          _compressTextures ? TextureStoreFormat.GuessCompressedFormat : TextureStoreFormat.GuessNoCompressedFormat,
          true);
    } else {
