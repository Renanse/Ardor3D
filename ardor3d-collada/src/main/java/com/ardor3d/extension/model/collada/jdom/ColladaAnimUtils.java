...
public class ColladaAnimUtils {
  ...
  /**
   * Clone the given MeshData object via deep copy using the Ardor3D BinaryExporter and
   * BinaryImporter.
   *
   * @param meshData
   *          the source to clone.
  ...
  void buildController(final Node node, final Element instanceController) {
    ...
    if (controller == null) {
      throw new ColladaException("Unable to find controller with id: " + instanceController.getAttributeValue("url"),
          instanceController);
    }
  ...
  /**
   * Construct skin mesh(es) from the skin element and attach them (under a single new Node) to the
   * given parent Node.
   *
   ...
   *          our <skin> element.
   */
  private void buildSkinMeshes(final Node ardorParentNode, final Element instanceController, final Element controller,
      final Element skin) {
    ...
    if (skinNodeEL == null || !"geometry".equals(skinNodeEL.getName())) {
      throw new ColladaException(
          "Expected a mesh for skin source with url: " + skinSource + " got instead: " + skinNodeEL, skin);
    }
    ...
    if (meshNode != null) {
      ...
      for (final Element sk : instanceController.getChildren("skeleton")) {
        ...
        if (skroot != null) {
          ...
          throw new ColladaException(
              "Unable to find node with id: " + sk.getText() + ", referenced from skeleton " + sk, sk);
        }
      }
      ...
      // Use the skeleton information from the instance_controller to set the parent array locations on
      // the
      // joints.
      ...
      for (int i = 0; i < jointNames.size(); i++) {
        ...
        if (found == null) {
          if (paramType == ParamType.idref_param) {
            ...
            found = (Element) _colladaDOMUtil.selectSingleNode(geometry, "/*//visual_scene//*[@sid='" + name + "']");
          }
          ...
          if (found == null) {
            found = (Element) _colladaDOMUtil.selectSingleNode(geometry, "/*//visual_scene//*[@name='" + name + "']");
          }
      ...
      for (final Spatial spat : meshNode.getChildren()) {
        if (spat instanceof Mesh sourceMesh && ((Mesh) spat).getMeshData().getVertexCount() > 0) {
  ...
  /**
   * Construct morph mesh(es) from the <morph> element and attach them (under a single new Node) to
   * the given parent Node.
   *
   * Note: This method current does not do anything but attach the referenced mesh since Ardor3D does
   * not yet support morph target animation.
   *
   * @param ardorParentNode
  ...
  private void buildMorphMeshes(final Node ardorParentNode, final Element controller, final Element morph) {
    ...
    if (skinNode == null || !"geometry".equals(skinNode.getName())) {
      throw new ColladaException(
          "Expected a mesh for morph source with url: " + skinSource + " (line number is referring morph)", morph);
    }
  ...
  private void parseAnimations(final Multimap<Element, TargetChannel> channelMap, final Element animationRoot,
    ...
    if (animationRoot.getChild("channel") != null) {
      ...
      for (final Element channel : channels) {
        ...
        channelMap.put(targetNode.getParentElement(), new TargetChannel(target, targetNode, source, animationItemRoot));
      }
    }
  }
