...
public class ParticleEmissionPanel extends ParticleEditPanel {
  ...
  public ParticleEmissionPanel() {
    ...
    _directionPanel.addChangeListener(e -> {
    ...
    _directionPanel.add(_rotateWithEmitterBox, new GridBagConstraints(0, 2, 1, 1, 1.0, 0.0, GridBagConstraints.CENTER,
        GridBagConstraints.NONE, new Insets(5, 5, 5, 5), 0, 0));

    _minAnglePanel.addChangeListener(
        e -> getEdittedParticles().setMinimumAngle(_minAnglePanel.getDoubleValue() * MathUtils.DEG_TO_RAD));
    _maxAnglePanel.addChangeListener(
        e -> getEdittedParticles().setMaximumAngle(_maxAnglePanel.getDoubleValue() * MathUtils.DEG_TO_RAD));
    ...
    _velocityPanel.addChangeListener(e -> getEdittedParticles().setInitialVelocity(_velocityPanel.getDoubleValue()));
    ...
    _translationPanel.addChangeListener(e -> getEdittedParticles().setTranslation(_translationPanel.getValue()));

    _rotationPanel.setBorder(createTitledBorder(" ROTATION "));
    _rotationPanel.addChangeListener(e -> {
    ...
    _scalePanel.addChangeListener(e -> getEdittedParticles().setScale(_scalePanel.getDoubleValue()));
    ...
    _originTypeBox = new JComboBox<>(EmitTypes.values());
    _originTypeBox.setBorder(createTitledBorder(" EMITTER TYPE "));
    _originTypeBox.addActionListener(e -> updateOriginParams());
  ...
  private JPanel createLineParamsPanel() {
    ...
    _lineLengthPanel.addChangeListener(e -> {
  ...
  private JPanel createRectParamsPanel() {
    _rectWidthPanel.addChangeListener(e -> {
    ...
    _rectHeightPanel.addChangeListener(e -> {
      final Rectangle3 rect = ((RectangleEmitter) getEdittedParticles().getParticleEmitter()).getSource();
  ...
  private JPanel createRingParamsPanel() {
    _ringInnerPanel.addChangeListener(e -> {
    ...
    _ringOuterPanel.addChangeListener(e -> {
      final Ring ring = ((RingEmitter) getEdittedParticles().getParticleEmitter()).getSource();
