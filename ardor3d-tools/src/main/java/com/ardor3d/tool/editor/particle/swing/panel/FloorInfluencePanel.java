...
public class FloorInfluencePanel extends InfluenceEditPanel {
    ...
    public FloorInfluencePanel() {
        ...
        _constantValue.addChangeListener(new ChangeListener() {

            public void stateChanged(final ChangeEvent e) {
                ((FloorInfluence) getEdittedInfluence()).setConstant(_constantValue.getDoubleValue());
            }
        });
        add(_constantValue, BorderLayout.NORTH);

        _normalVector.setBorder(createTitledBorder(" PLANE NORMAL "));
        _normalVector.addChangeListener(new ChangeListener() {

            public void stateChanged(final ChangeEvent e) {
                ((FloorInfluence) getEdittedInfluence()).setNormal(_normalVector.getValue());
            }
        });
        add(_normalVector, BorderLayout.CENTER);

        _bouncinessValue.addChangeListener(new ChangeListener() {

            public void stateChanged(final ChangeEvent e) {
                ((FloorInfluence) getEdittedInfluence()).setBounciness(_bouncinessValue.getDoubleValue());
            }
        });
