...
public abstract class ParticleAppearancePanel extends ParticleEditPanel {
  ...
  private final DefaultListModel<RampEntry> _rampModel = new DefaultListModel<>();
  ...
  private final DefaultListModel<AnimationEntry> _animModel = new DefaultListModel<>();
  ...
  public ParticleAppearancePanel(final Preferences prefs) {
    ...
    _geomTypeBox =
        new JComboBox<>(new ParticleType[] {ParticleType.Triangle, ParticleType.Line, ParticleType.Point});
    _geomTypeBox.addActionListener(e -> changeParticleType((ParticleType) _geomTypeBox.getSelectedItem()));
    ...
    _parentDirectionLeftPanel
        .addChangeListener(e -> getEdittedParticles().setFacingLeftVector(_parentDirectionLeftPanel.getValue()));
    ...
    _parentDirectionUpPanel
        .addChangeListener(e -> getEdittedParticles().setFacingUpVector(_parentDirectionUpPanel.getValue()));
    ...
    _rampList = new JList<>(_rampModel);
    _rampList.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
    _rampList.addListSelectionListener(e -> {
    ...
    _rampAddButton.addActionListener(e -> new Thread() {
      @Override
      public void run() {
    ...
    _rampEditButton.addActionListener(e -> new Thread() {
      @Override
      public void run() {
    ...
    _rampRemoveButton.addActionListener(e -> {
      final RampEntry entry = _rampList.getSelectedValue();
      getEdittedParticles().getRamp().removeEntry(entry);
      updateRampModel();
    });

    _rampMoveUpButton.addActionListener(e -> {
      final int index = _rampList.getSelectedIndex();
      final RampEntry entry = _rampList.getSelectedValue();
      getEdittedParticles().getRamp().removeEntry(entry);
    ...
    _rampMoveDownButton.addActionListener(e -> {
      final int index = _rampList.getSelectedIndex();
      final RampEntry entry = _rampList.getSelectedValue();
      getEdittedParticles().getRamp().removeEntry(entry);
    ...
    _animList = new JList<>(_animModel);
    _animList.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
    _animList.addListSelectionListener(e -> {
    ...
    _animAddButton.addActionListener(e -> new Thread() {
      @Override
      public void run() {
    ...
    _animEditButton.addActionListener(e -> new Thread() {
      @Override
      public void run() {
    ...
    _animRemoveButton.addActionListener(e -> {
      final AnimationEntry entry = _animList.getSelectedValue();
      getEdittedParticles().getTexAnimation().removeEntry(entry);
      updateAnimModel();
    });

    _animMoveUpButton.addActionListener(e -> {
      final int index = _animList.getSelectedIndex();
      final AnimationEntry entry = _animList.getSelectedValue();
      getEdittedParticles().getTexAnimation().removeEntry(entry);
    ...
    _animMoveDownButton.addActionListener(e -> {
      final int index = _animList.getSelectedIndex();
      final AnimationEntry entry = _animList.getSelectedValue();
      getEdittedParticles().getTexAnimation().removeEntry(entry);
      getEdittedParticles().getTexAnimation().addEntry(index + 1, entry);
    ...
    _texPanel.addChangeListener(e -> getEdittedParticles().setTexQuantity(_texPanel.getIntValue()));
    ...
    _startTexPanel.addChangeListener(e -> getEdittedParticles().setStartTexIndex(_startTexPanel.getIntValue()));
    ...
    add(blendPanel, new GridBagConstraints(0, 3, 1, 1, 1.0, 0.0, GridBagConstraints.CENTER,
        GridBagConstraints.HORIZONTAL, new Insets(5, 10, 5, 10), 0, 0));
    add(rampPanel, new GridBagConstraints(0, 4, 1, 1, 1.0, 1.0, GridBagConstraints.NORTH, GridBagConstraints.HORIZONTAL,
        new Insets(5, 10, 5, 10), 0, 0));
    add(animPanel, new GridBagConstraints(0, 5, 1, 1, 1.0, 1.0, GridBagConstraints.NORTH, GridBagConstraints.HORIZONTAL,
        new Insets(5, 10, 5, 10), 0, 0));
  ...
  private void changeTexture() {
    try {
      ...
      GameTaskQueueManager.getManager(ParticleEditorFrame.GLOBAL_CONTEXT).render(() -> {
        loadApplyTexture();
        return null;
      });
      ...
      try {
        ...
        if (tex != null) {
          if (tex.getTextureKey() != null && tex.getTextureKey().getSource() != null) {
            _imageLabel.setIcon(new ImageIcon(((URLResourceSource) tex.getTextureKey().getSource()).getURL()));
          }
        } else {
          _imageLabel.setIcon(null);
        }
