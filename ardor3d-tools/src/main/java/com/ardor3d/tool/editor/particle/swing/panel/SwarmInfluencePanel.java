...
public class SwarmInfluencePanel extends InfluenceEditPanel {
  ...
  private final ValuePanel _swarmTurnSpeed =
      new ValuePanel("Turn Speed: ", "", -Double.MIN_VALUE, Double.MAX_VALUE, 0.1);
  private final ValuePanel _swarmMaxSpeed = new ValuePanel("Max Speed: ", "", -Double.MIN_VALUE, Double.MAX_VALUE, 0.1);
  private final ValuePanel _swarmAcceleration =
      new ValuePanel("Acceleration: ", "", -Double.MIN_VALUE, Double.MAX_VALUE, 0.1);
  ...
  public SwarmInfluencePanel() {
    ...
    _swarmRange
        .addChangeListener(e -> ((SwarmInfluence) getEdittedInfluence()).setSwarmRange(_swarmRange.getDoubleValue()));
    _swarmDeviance.addChangeListener(e -> ((SwarmInfluence) getEdittedInfluence())
        .setDeviance(MathUtils.DEG_TO_RAD * _swarmDeviance.getDoubleValue()));
    _swarmMaxSpeed
        .addChangeListener(e -> ((SwarmInfluence) getEdittedInfluence()).setMaxSpeed(_swarmMaxSpeed.getDoubleValue()));
    _swarmAcceleration.addChangeListener(
        e -> ((SwarmInfluence) getEdittedInfluence()).setSpeedBump(_swarmAcceleration.getDoubleValue()));
    _swarmTurnSpeed.addChangeListener(
        e -> ((SwarmInfluence) getEdittedInfluence()).setTurnSpeed(_swarmTurnSpeed.getDoubleValue()));

    _swarmLocationPanel.addChangeListener(
        e -> ((SwarmInfluence) getEdittedInfluence()).setSwarmOffset(_swarmLocationPanel.getValue()));
