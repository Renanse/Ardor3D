...
public class ParticleWorldPanel extends ParticleEditPanel {
  ...
  private final JComboBox<RenderBucketType> _renderBucketCB = new JComboBox<>(new RenderBucketType[] {
      RenderBucketType.Inherit, RenderBucketType.Transparent, RenderBucketType.Opaque, RenderBucketType.OrthoOrder});
  ...
  public ParticleWorldPanel() {
    ...
    _speedPanel
        .addChangeListener(e -> getEdittedParticles().getParticleController().setSpeed(_speedPanel.getDoubleValue()));

    _precisionPanel.setBorder(createTitledBorder("UPDATE PRECISION"));
    _precisionPanel.addChangeListener(
        e -> getEdittedParticles().getParticleController().setPrecision(_precisionPanel.getDoubleValue()));

    final JLabel queueLabel = createBoldLabel("Render Bucket:");

    _renderBucketCB.addActionListener(e -> getEdittedParticles().getSceneHints()
