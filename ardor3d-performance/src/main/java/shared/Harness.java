...
public class Harness {
  ...
  private static void runTests(final MatrixBenchmark[] benchmarks, final int loops, final boolean print) {
    Harness.printResults("Transform Multiply", Harness.runTests(benchmarks, loops, "TransformMult"), benchmarks, print);
    Harness.printResults("4x4 Multiplication - Ma*Mb", Harness.runTests(benchmarks, loops, "Mult"), benchmarks, print);
    ...
    Harness.printResults("Rotation (Arbitrary axis) - M*Mr", Harness.runTests(benchmarks, loops, "Rotate1"), benchmarks,
        print);
    ...
    Harness.printResults("Vector Transformation (M*v)", Harness.runTests(benchmarks, loops, "TransformVector"),
        benchmarks, print);
    Harness.printResults("Translation of Matrix4", Harness.runTests(benchmarks, loops, "Translate"), benchmarks, print);
    Harness.printResults("Transpose of Matrix4", Harness.runTests(benchmarks, loops, "Transpose"), benchmarks, print);
  }

  private static void printResults(final String label, final TestResults[] results, final MatrixBenchmark[] benchmarks,
    ...
    for (int i = 0; i < results.length; i++) {
      if (results[i] != null) {
        System.out.format("\t" + benchmarks[i].getPlatformName() + ": Avg: %,.0f ips, Min: %,.0f ips, Max: %,.0f ips%n",
            results[i].avg, results[i].min, results[i].max);
      } else {
