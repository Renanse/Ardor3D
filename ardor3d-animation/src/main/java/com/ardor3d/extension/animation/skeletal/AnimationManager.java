...
/**
 * <p>
 * AnimationManager describes and maintains an animation system. It tracks one or more layered
 * animation state machines (AnimationLayer) and uses their combined result to update one or more
 * poses (via a set AnimationApplier.) AnimationClips used in these layers are instanced and tracked
 * specifically for this manager.
 * </p>
 * <p>
 * By default, an animation manager has a single base animation layer. Other layers may be added to
 * this. It is important that the base layer (the layer at index 0) always has a full set of data to
 * put a skeleton pose into a valid state.
 * </p>
 */
public class AnimationManager {
  ...
  /**
   * A timer to use as our "global" time keeper. All animation sources under this manager will use
   * this timer as their time reference.
   */
  ...
  /**
   * A map of key->Double values, allowing control over elements under this manager without needing
   * precise knowledge of the layout of those layers, blend trees, etc. Missing keys will return 0.0
   * and log a warning.
   */
   ...
   */
  public double getCurrentGlobalTime() { return _globalTimer.getTimeInSeconds(); }

  /**
   * @return the timer used by this manager for global time keeping.
   */
  public ReadOnlyTimer getGlobalTimer() { return _globalTimer; }

  /**
   * @param timer
   *          the timer to be used by this manager for global time keeping.
   */
  public void setGlobalTimer(final Timer timer) { _globalTimer = timer; }

  /**
   *
   * @return True if clips will reset if the currentUpdateState is Stop.
   */
  public boolean isResetClipsOnStop() { return _resetClipsOnStop; }
   ...
   *
   */
  public void setResetClipsOnStop(final boolean resetClipsOnStop) { _resetClipsOnStop = resetClipsOnStop; }
  ...
  public boolean isPlaying() { return _currentAnimationState == AnimationUpdateState.Play; }

  public boolean isPaused() { return _currentAnimationState == AnimationUpdateState.Pause; }

  public boolean isStopped() { return _currentAnimationState == AnimationUpdateState.Stop; }
   ...
   */
  public int getPoseCount() { return _applyToPoses.size(); }
   ...
   */
  public AnimationApplier getApplier() { return _applier; }

  /**
   * @param applier
   *          a logic object to be responsible for taking animation data and applying it to skeleton
   *          poses.
   */
  public void setApplier(final AnimationApplier applier) { _applier = applier; }

  /**
   * Move associated layers forward to the current global time and then apply the associated animation
   * data to any SkeletonPoses set on the manager.
   */
  public void update() {
   ...
   * @return the animation layer at that index, or null of index is outside the bounds of our list of
   *         layers.
   */
   ...
   */
  public AnimationLayer getBaseAnimationLayer() { return _layers.get(0); }

  /**
   * @return the amount of time in seconds between frame rate updates. (throttle) default is 60fps
   *         (1.0/60.0).
   */
  public double getUpdateRate() { return _updateRate; }

  /**
   * @param updateRate
   *          the new throttle rate. Default is 60fps (1.0/60.0). Set to 0 to disable throttling.
   */
  public void setUpdateRate(final double updateRate) { _updateRate = updateRate; }
  ...
  public LoggingMap<String, Double> getValuesStore() { return _valuesStore; }

  /**
   * @return the Map containing the AnimationClips and their respective AnimationClipInstances.
   */
  public Map<AnimationClip, AnimationClipInstance> getClipInstancesStore() { return _clipInstances; }
}
