...
public class SkinnedMesh extends Mesh implements PoseListener {
  ...
  /**
   * Storage for per vertex joint indices. These should already be normalized (all joints affecting
   * the vertex add to 1.) There should be "weightsPerVert" entries per vertex.
   */
  protected float[] _weights;

  /**
   * The original bind pose form of this SkinnedMesh. When doing CPU skinning, this will be used as a
   * source and the destination will go into the normal _meshData field for rendering. For GPU
   * skinning, _meshData will be ignored and only _bindPose will be sent to the card.
   */
  ...
  /**
   * <p>
   * Flag for enabling automatically updating the skin's model bound when the pose changes. Only
   * effective in CPU skinning mode. Default is false as this is currently expensive.
   * </p>
   *
   * XXX: If we can find a better way to update the bounds, maybe we should make this default to true
   ...
   */
  public MeshData getBindPoseData() { return _bindPoseData; }
   ...
   *          the new bind pose
   */
  public void setBindPoseData(final MeshData poseData) { _bindPoseData = poseData; }

  /**
   * @return the number of weights and jointIndices this skin uses per vertex.
   */
  public int getWeightsPerVert() { return _weightsPerVert; }
   ...
   */
  public void setWeightsPerVert(final int weightsPerVert) { _weightsPerVert = weightsPerVert; }

  /**
   * @return true if we should use a matrix to send joints and weights to a gpu shader.
   */
  public boolean isGpuUseMatrixAttribute() { return _gpuUseMatrixAttribute; }

  /**
   * @param useMatrix
   *          true if we should use a matrix to send joints and weights to a gpu shader.
   */
  public void setGpuUseMatrixAttribute(final boolean useMatrix) { _gpuUseMatrixAttribute = useMatrix; }

  /**
   * @return size to pad our attributes to. If we are using matrices (see
   *         {@link #setGpuUseMatrixAttribute(boolean)}) then this is the size of an edge of the
   *         matrix. eg. 4 would mean either a vec4 or a mat4 object is expected in the shader.
   */
  public int getGpuAttributeSize() { return _gpuAttributeSize; }
   ...
   *          matrix. eg. 4 would mean either a vec4 or a mat4 object is expected in the shader.
   */
  public void setGpuAttributeSize(final int size) { _gpuAttributeSize = size; }

  /**
   * @return this skinned mesh's joint influences as indices into a Skeleton's Joint array.
   * @see #setJointIndices(short[])
   */
  public short[] getJointIndices() { return _jointIndices; }

  /**
   * Sets the joint indices used by this skinned mesh to compute mesh deformation. Each entry is
   * interpreted as an 16bit signed integer index into a Skeleton's Joint.
   *
   * @param jointIndices
   */
   ...
   */
  public SkeletonPose getCurrentPose() { return _currentPose; }
  ...
  /**
   * @return true if we should automatically update our model bounds when our pose updates. If useGPU
   *         is true, bounds are ignored.
   */
  public boolean isAutoUpdateSkinBounds() { return _autoUpdateSkinBound; }

  /**
   * @param autoUpdateSkinBound
   *          true if we should automatically update our model bounds when our pose updates. If useGPU
   *          is true, bounds are ignored.
   */
   ...
   */
  public boolean isUseGPU() { return _useGPU; }
   ...
   */
  public void setUseGPU(final boolean useGPU) { _useGPU = useGPU; }

  /**
   * @return any custom apply logic set on this skin or null if default logic is used.
   * @see #setCustomApplier(SkinPoseApplyLogic)
   */
  public SkinPoseApplyLogic getCustomApplier() { return _customApplier; }

  /**
   * Set custom logic for how this skin should react when it is told its pose has updated. This might
   * include throttling skin application, ignoring skin application when the skin is outside of the
   * camera view, etc. If null, (the default) the skin will always apply the new pose and optionally
   * update the model bound.
   *
   * @param customApplier
   *          the new custom logic, or null to use the default behavior.
   */
  public void setCustomApplier(final SkinPoseApplyLogic customApplier) { _customApplier = customApplier; }
  ...
  public void applyPose() {
    ...
    for (int i = 0; i < _bindPoseData.getVertexCount(); i++) {
      ...
      for (int j = 0; j < getWeightsPerVert(); j++) {
        ...
        tempX = jntMat.getM00() * bindVX + jntMat.getM01() * bindVY + jntMat.getM02() * bindVZ + jntMat.getM03();
        tempY = jntMat.getM10() * bindVX + jntMat.getM11() * bindVY + jntMat.getM12() * bindVZ + jntMat.getM13();
        tempZ = jntMat.getM20() * bindVX + jntMat.getM21() * bindVY + jntMat.getM22() * bindVZ + jntMat.getM23();
  ...
  public void recreateJointAttributeBuffer() {
    ...
    if (isGpuUseMatrixAttribute()) {
      data =
          SkinUtils.reorderAndPad(SkinUtils.convertToFloat(_jointIndices), getWeightsPerVert(), getGpuAttributeSize());
    } else {
  ...
  public void recreateWeightAttributeBuffer() {
    ...
    _bindPoseData.setCoords("weights", new FloatBufferData(BufferUtils.createFloatBuffer(data), getGpuAttributeSize()));
  }
  ...
  /**
   * Rewrites the weights on this SkinnedMesh, if necessary, to reduce the number of weights per vert
   * to the given max. This is done by dropping the least significant weight and balancing the
   * remainder to total 1.0 again.
   *
   * @param maxCount
   *          the desired maximum weightsPerVert. If this is >= the current weightsPerVert, this
   *          method is a NOOP.
   */
  ...
  @Override
  public Class<? extends SkinnedMesh> getClassTag() { return this.getClass(); }
