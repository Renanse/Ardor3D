...
public class TransformChannel extends AbstractAnimationChannel {
  ...
  // XXX: Perhaps we could optimize memory by reusing sample objects that are the same from one index
  // to the next.
  // XXX: Could then also optimize execution time by checking object equality (==) and skipping
  // (s)lerps.
   ...
   *          our time offset values.
   * @param transforms
   *          the transform to set on this channel at each time offset. These are separated into
   *          rotation, scale and translation components. Note that supplying transforms with
   *          non-rotational matrices (with built in shear, scale.) will produce a warning and may not
  ...
  @Override
  public Class<? extends TransformChannel> getClassTag() { return this.getClass(); }
