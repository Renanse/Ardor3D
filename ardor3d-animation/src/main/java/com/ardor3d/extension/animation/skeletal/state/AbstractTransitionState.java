...
public abstract class AbstractTransitionState extends AbstractFiniteState {
  ...
  /**
   * @return the name of the steady state we want the Animation Layer to be in at the end of the
   *         transition.
   */
  public String getTargetState() { return _targetState; }

  /**
   * @param startWindow
   *          our new start window value. If greater than 0, this transition is only valid if the
   *          current time is >= startWindow. Note that animations are separate from states, so time
   *          scaling an animation will not affect transition windows directly and must be factored
   *          into the start/end values.
   */
  public void setStartWindow(final double startWindow) { _startWindow = startWindow; }

  /**
   * @return our start window value.
   * @see #setStartWindow(double)
   */
  public double getStartWindow() { return _startWindow; }

  /**
   * @param endWindow
   *          our new end window value. If greater than 0, this transition is only valid if the
   *          current time is <= endWindow. Note that animations are separate from states, so time
   *          scaling an animation will not affect transition windows directly and must be factored
   *          into the start/end values.
   */
  public void setEndWindow(final double endWindow) { _endWindow = endWindow; }

  /**
   * @return our end window value.
   * @see #setEndWindow(double)
   */
  public double getEndWindow() { return _endWindow; }
