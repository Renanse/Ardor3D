 ...
 * This source is meant to be used for controlling a particular joint or set of joints
 * programatically.
 */
public class ManagedTransformSource implements BlendTreeSource {
  ...
  /**
   * Setup transform data on this source, using the first frame from a specific clip and jointNames
   * from a specific pose.
   * 
   * @param pose
  ...
  public void initJointsByName(final SkeletonPose pose, final AnimationClip clip, final String... jointNames) {
    for (final String name : jointNames) {
      ...
      setJointTransformData(jointIndex,
          ((JointChannel) clip.findChannelByName(JointChannel.JOINT_CHANNEL_NAME + jointIndex)).getJointData(0,
              new JointData(jointIndex)));
    }
  }
  ...
  public void setSourceName(final String name) { sourceName = name; }
}
