...
/**
 * Base class for animation channels. An animation channel describes a single element of an
 * animation (such as the movement of a single joint, or the play back of a specific sound, etc.)
 * These channels are grouped together in an AnimationClip to describe a full animation.
 */
public abstract class AbstractAnimationChannel implements Savable {
   ...
   */
  public int getSampleCount() { return _times.length; }
   ...
   * @param applyTo
   *          the Object to apply to. The type of the object and what data is set will depend on the
   *          Channel subclass.
   */
  public void updateSample(final double clockTime, final Object applyTo) {
    ...
    if (clockTime < 0 || _times.length == 1) {
      ...
      final double progressPercent = (clockTime - _times[startFrame]) / (_times[startFrame + 1] - _times[startFrame]);
  ...
  /**
   * Sets data on the given applyTo Object for the given sampleIndex and a percent progress towards
   * the sample following it.
   * 
   * @param sampleIndex
  ...
  /**
   * Returns a new channel of the same name and content as this, but trimmed to just the times between
  ...
  /**
   * 
   * Returns a new channel of the same content as this, but trimmed to just the times between start
   * and end sample.
   * 
   * @param name
