...
public class GLFWCanvasCallback implements Lwjgl3CanvasCallback {
	...
	/**
	 * Construct a canvas callback for the supplied window. Event polling will be
	 * set to true.
	 * 
	 * @param windowId supplies the window id of an underlying glfw based canvas.
	 */
	GLFWCanvasCallback(LongSupplier windowId) {
		this(windowId, true);
	}
	 ...
	 * 
	 * @param windowId       supplies the window id of an underlying glfw based
	 *                       canvas.
	 * @param doEventPolling if true, poll glfw for events after swapping buffers.
	 */
	GLFWCanvasCallback(LongSupplier windowId, boolean doEventPolling) {
		_windowId = windowId;
